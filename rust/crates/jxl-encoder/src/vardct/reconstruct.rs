...
pub(crate) fn reconstruct_xyb(
    ...
    let mut output = core::array::from_fn(|_| jxl_simd::vec_f32_dirty(num_pixels));
...
pub(crate) fn gab_smooth_with_scratch(
        ...
        macro_rules! gab_smooth_plane {
            ($p:expr) => {{
                GAB_SCRATCH.with(|s| {
                    ...
                    jxl_simd::gab_smooth_channel($p, &mut s[..n], width, height, w_center, w1, w2);
                });
            }};
        }
    ...
    for plane in planes.iter_mut() {
        jxl_simd::gab_smooth_channel(plane, scratch, width, height, w_center, w1, w2);
    }
}

pub(crate) fn gab_smooth(planes: &mut [Vec<f32>; 3], width: usize, height: usize) {
    let trace = crate::bench_trace_enabled();
