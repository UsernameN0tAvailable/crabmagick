...
use crate::container::{wrap_in_container, wrap_in_container_jxlp};
...
use crate::parallel::parallel_map_result;
use crate::vardct::ac_context;
use crate::vardct::ac_group::{collect_ac_coefficients_dct8_into, predict_from_top_and_left};
...
pub fn encode_jpeg_to_jxl_sequential(jpeg: &JpegData) -> Result<Vec<u8>> {
    crate::parallel::with_sequential_maps(|| encode_jpeg_to_jxl(jpeg))
}
...
fn encode_jpeg_to_jxl_inner(jpeg: &JpegData) -> Result<(Vec<u8>, usize)> {
    ...
    let (dc_code, ac_code) = if crate::parallel::sequential_maps_forced() {
        (
...
pub fn encode_jpeg_to_jxl_container_sequential(jpeg: &JpegData) -> Result<Vec<u8>> {
    crate::parallel::with_sequential_maps(|| encode_jpeg_to_jxl_container(jpeg))
}
