use crabmagick_core::processor::{ChromaSubsampling, EncodeOptions, JpegEncodeOptions};
use crabmagick_core::{
    init,
    pipeline::{DecodedImage, decode_any_with_options, encode},
};
use std::io::Write;
...
fn main() {
    ...
    for &w in &[
        64u32, 80, 96, 112, 128, 144, 160, 176, 192, 208, 224, 240, 256,
    ] {
        let h = w;
        let mcu420 = (w + 15) / 16;
        let mcu444 = (w + 7) / 8;
        ...
        println!(
            "{w:3}  {mcu420:>3}({:>4}px)  {p420:>8.1}  {mcu444:>3}({:>4}px)  {p444:>8.1}",
            mcu420 * 16,
            mcu444 * 8
        );
    }
