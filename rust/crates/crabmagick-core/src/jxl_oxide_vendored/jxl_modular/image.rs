...
use crate::jxl_oxide_vendored::jxl_coding::{Decoder, DecoderRleMode, RleToken};
        ...
        for (i, (info, grid)) in it {
            ...
            let (groups, grids) = if hshift < 3 || vshift < 3 {
                ...
                if group_width == 0 || group_height == 0 {
                    ...
                    return Err(crate::jxl_oxide_vendored::jxl_modular::Error::InvalidSqueezeParams);
                }
...
impl<'dest, S: Sample> TransformedModularSubimage<'dest, S> {
    ...
    pub fn recursive(
        ...
        let channels = crate::jxl_oxide_vendored::jxl_modular::ModularChannels {
            info: self.channel_info,
            nb_meta_channels: 0,
        };
        let (header, ma_ctx) = crate::jxl_oxide_vendored::jxl_modular::read_and_validate_local_modular_header(
            bitstream,
            &channels,
            global_ma_config,
            tracker,
    ...
    pub fn finish(mut self, pool: &crate::jxl_oxide_vendored::jxl_threadpool::JxlThreadPool) -> bool {
...
struct RleState<S: Sample> {
    value: S,
    repeat: u32,
    error: Option<Box<crate::jxl_oxide_vendored::jxl_coding::Error>>,
}
