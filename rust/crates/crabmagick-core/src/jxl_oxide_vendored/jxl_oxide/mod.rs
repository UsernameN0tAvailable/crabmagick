...
//!
//! ```no_run
//! # use crate::jxl_oxide_vendored::jxl_oxide::JxlImage;
...
//! #     type Output = crate::jxl_oxide_vendored::jxl_oxide::Result<&'static [u8]>;
//! #     fn poll(
...
//! # }
//! #
//! # use crate::jxl_oxide_vendored::jxl_oxide::{JxlImage, InitializeResult};
//! # async fn run() -> crate::jxl_oxide_vendored::jxl_oxide::Result<()> {
...
//!
//! ```no_run
//! # use crate::jxl_oxide_vendored::jxl_oxide::Render;
//! use crate::jxl_oxide_vendored::jxl_oxide::{JxlImage, RenderResult};
//!
//! # fn present_image(_: Render) {}
//! # fn main() -> crate::jxl_oxide_vendored::jxl_oxide::Result<()> {
...
//!
//! ```no_run
//! # use crate::jxl_oxide_vendored::jxl_oxide::{EnumColourEncoding, JxlImage, RenderingIntent};
//! # use crate::jxl_oxide_vendored::jxl_oxide::NullCms as MyCustomCms;
//! # let reader = std::io::empty();
...
//!
//! ```no_run
//! # use crate::jxl_oxide_vendored::jxl_oxide::{JxlImage, NullCms};
//! # let reader = std::io::empty();
//! let mut image = JxlImage::builder().read(reader).expect("Failed to read image header");
...
use crate::jxl_oxide_vendored::jxl_bitstream::{Bitstream, ContainerParser, ParseEvent};
use crate::jxl_oxide_vendored::jxl_frame::FrameContext;
use crate::jxl_oxide_vendored::jxl_image::BitDepth;
use crate::jxl_oxide_vendored::jxl_oxide_common::{Bundle, Name};
use crate::jxl_oxide_vendored::jxl_render::ImageBuffer;
use crate::jxl_oxide_vendored::jxl_render::ImageWithRegion;
use crate::jxl_oxide_vendored::jxl_render::Region;
use crate::jxl_oxide_vendored::jxl_render::{IndexedFrame, RenderContext};

pub use crate::jxl_oxide_vendored::jxl_color::{ColorEncodingWithProfile, ColorManagementSystem, NullCms, PreparedTransform};
pub use crate::jxl_oxide_vendored::jxl_frame::header as frame;
pub use crate::jxl_oxide_vendored::jxl_frame::{Frame, FrameHeader};
pub use crate::jxl_oxide_vendored::jxl_grid::{AlignedGrid, AllocTracker};
pub use crate::jxl_oxide_vendored::jxl_image::color::{self, EnumColourEncoding, RenderingIntent};
pub use crate::jxl_oxide_vendored::jxl_image::{self as image, ExtraChannelType, ImageHeader};
pub use crate::jxl_oxide_vendored::jxl_jbr as jpeg_bitstream;
pub use crate::jxl_oxide_vendored::jxl_threadpool::JxlThreadPool;
...
/// # Examples
/// ```no_run
/// # fn read_bytes() -> crate::jxl_oxide_vendored::jxl_oxide::Result<&'static [u8]> { Ok(&[]) }
/// # use crate::jxl_oxide_vendored::jxl_oxide::{JxlImage, InitializeResult};
/// # fn main() -> crate::jxl_oxide_vendored::jxl_oxide::Result<()> {
/// let mut uninit_image = JxlImage::builder().build_uninit();
/// let image = loop {
...
impl UninitializedJxlImage {
    ...
    pub fn try_init(mut self) -> Result<InitializeResult> {
        ...
        let embedded_icc = if image_header.metadata.colour_encoding.want_icc() {
            let icc = match crate::jxl_oxide_vendored::jxl_color::icc::read_icc(&mut bitstream) {
                Ok(x) => x,
                Err(e) if e.unexpected_eof() => {
                    return Ok(InitializeResult::NeedMoreData(self));
                }
            ...
            let icc = crate::jxl_oxide_vendored::jxl_color::icc::decode_icc(&icc)?;
            Some(icc)
        } else {
            None
        };
    ...
    pub fn rendered_icc(&self) -> Vec<u8> {
        ...
        match encoding.encoding() {
            color::ColourEncoding::Enum(encoding) => {
                crate::jxl_oxide_vendored::jxl_color::icc::colour_encoding_to_icc(encoding)
            }
...
impl JxlImage {
    ...
    pub fn jpeg_reconstruction_status(&self) -> JpegReconstructionStatus {
        match self.inner.aux_boxes.jbrd() {
            ...
            AuxBoxData::Decoding => {
                ...
                let frame_header = frame.header();
                if frame_header.encoding != crate::jxl_oxide_vendored::jxl_frame::header::Encoding::VarDct {
                    return JpegReconstructionStatus::Invalid;
                }
    ...
    pub fn reconstruct_jpeg(&self, jpeg_output: impl std::io::Write) -> Result<()> {
        ...
        let jbrd = match aux_boxes.jbrd() {
            ...
            AuxBoxData::Decoding => {
                return Err(crate::jxl_oxide_vendored::jxl_jbr::Error::ReconstructionDataIncomplete.into());
            }
            AuxBoxData::NotFound => {
                return Err(crate::jxl_oxide_vendored::jxl_jbr::Error::ReconstructionUnavailable.into());
            }
        };
        if self.num_loaded_frames() == 0 {
            return Err(crate::jxl_oxide_vendored::jxl_jbr::Error::FrameDataIncomplete.into());
        }
...
impl From<CropInfo> for crate::jxl_oxide_vendored::jxl_render::Region {
...
impl From<crate::jxl_oxide_vendored::jxl_render::Region> for CropInfo {
    fn from(value: crate::jxl_oxide_vendored::jxl_render::Region) -> Self {
        Self {
