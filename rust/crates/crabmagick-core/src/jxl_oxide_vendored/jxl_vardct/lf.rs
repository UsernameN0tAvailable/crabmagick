use crate::jxl_oxide_vendored::jxl_bitstream::{Bitstream, U};
use crate::jxl_oxide_vendored::jxl_grid::AllocTracker;
use crate::jxl_oxide_vendored::jxl_modular::{ChannelShift, MaConfig, Modular, ModularParams, Sample};
use crate::jxl_oxide_vendored::jxl_oxide_common::{Bundle, define_bundle};

use crate::jxl_oxide_vendored::jxl_vardct::Result;

define_bundle! {
    /// Dequantization information for each channel.
    #[derive(Debug)]
    pub struct LfChannelDequantization error(crate::jxl_oxide_vendored::jxl_vardct::Error) {
    ...
    #[derive(Debug)]
    pub struct Quantizer error(crate::jxl_oxide_vendored::jxl_vardct::Error) {
    ...
    #[derive(Debug)]
    pub struct LfChannelCorrelation error(crate::jxl_oxide_vendored::jxl_vardct::Error) {
        all_default: ty(Bool) default(true),
...
impl<Ctx> Bundle<Ctx> for HfBlockContext {
    type Error = crate::jxl_oxide_vendored::jxl_vardct::Error;

    fn parse(bitstream: &mut Bitstream, _: Ctx) -> crate::jxl_oxide_vendored::jxl_vardct::Result<Self> {
        ...
        let (num_block_clusters, block_ctx_map) = if bitstream.read_bool()? {
            ...
            for thr in &mut lf_thresholds {
                ...
                for _ in 0..num_lf_thresholds {
                    ...
                    let t = crate::jxl_oxide_vendored::jxl_bitstream::unpack_signed(t);
                    thr.push(t);
                }
            }
            ...
            let (num_clusters, ctx_map) = crate::jxl_oxide_vendored::jxl_coding::read_clusters(bitstream, bsize * 39)?;
            if num_clusters > 16 {
...
pub struct LfCoeffParams<'ma, 'pool, 'tracker> {
    ...
    pub pool: &'pool crate::jxl_oxide_vendored::jxl_threadpool::JxlThreadPool,
}
