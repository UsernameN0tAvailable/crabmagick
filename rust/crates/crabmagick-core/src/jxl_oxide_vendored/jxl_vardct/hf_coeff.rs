...
use crate::jxl_oxide_vendored::jxl_grid::{AllocTracker, MutableSubgrid, SharedSubgrid};
use crate::jxl_oxide_vendored::jxl_modular::{ChannelShift, Sample};

use crate::jxl_oxide_vendored::jxl_vardct::{BlockInfo, HfBlockContext, HfPass, Result};
...
pub fn write_hf_coeff<S: Sample>(
    ...
    if hfp >= num_hf_presets {
        ...
        return Err(
            crate::jxl_oxide_vendored::jxl_bitstream::Error::ValidationFailed("selected HF preset out of bounds").into(),
        );
    }
        ...
        for x in 0..width {
            ...
            for c in 0..3 {
                ...
                if non_zeros > (63 << num_blocks_log) {
                    ...
                    return Err(
                        crate::jxl_oxide_vendored::jxl_bitstream::Error::ValidationFailed("non_zeros too large").into(),
                    );
                }
                ...
                for (idx, &coeff_coord) in order[num_blocks as usize..].iter().enumerate() {
                    ...
                    let cluster = *cluster_map.get(coeff_ctx as usize).ok_or_else(|| {
                        ...
                        crate::jxl_oxide_vendored::jxl_bitstream::Error::ValidationFailed(
                    ...
                    let coeff = crate::jxl_oxide_vendored::jxl_bitstream::unpack_signed(ucoeff) << coeff_shift;
