...
use crate::jxl_oxide_vendored::jxl_grid::{AlignedGrid, AllocTracker};
use crate::jxl_oxide_vendored::jxl_modular::{MaConfig, Modular, ModularChannelParams, ModularParams};
            ...
            while x < bw {
                if !block_info.get(x, y).is_occupied() {
                    ...
                    if hf_mul <= 0 {
                        ...
                        return Err(
                            crate::jxl_oxide_vendored::jxl_bitstream::Error::ValidationFailed("non-positive HfMul").into()
                        );
                    }
