...
fn tone_map_generic(
    ...
    for ((r, g), b) in r.iter_mut().zip(g).zip(b) {
        ...
        let y_pq = crate::jxl_oxide_vendored::jxl_color::tf::pq::linear_to_pq_generic(y, intensity_target);
        let y_mapped = crate::jxl_oxide_vendored::jxl_color::tf::rec2408::rec2408_eetf_generic(
            y_pq,
            intensity_target,
            from_luminance_range,
            to_luminance_range,
        );
        let y_mapped = crate::jxl_oxide_vendored::jxl_color::tf::pq::pq_to_linear_generic(y_mapped, intensity_target);
...
unsafe fn tone_map_x86_64_avx2(
    ...
    for ((r, g), b) in (&mut r).zip(&mut g).zip(&mut b) {
        ...
        let vy_pq = crate::jxl_oxide_vendored::jxl_color::tf::pq::linear_to_pq_x86_64_avx2(vy, intensity_target);
        let vy_mapped = crate::jxl_oxide_vendored::jxl_color::tf::rec2408::rec2408_eetf_x86_64_avx2(
            vy_pq,
            intensity_target,
            from_luminance_range,
            to_luminance_range,
        );
        let vy_mapped = crate::jxl_oxide_vendored::jxl_color::tf::pq::pq_to_linear_x86_64_avx2(vy_mapped, intensity_target);
        let is_small =
...
unsafe fn tone_map_x86_64_fma(
    ...
    for ((r, g), b) in (&mut r).zip(&mut g).zip(&mut b) {
        ...
        let vy_pq = crate::jxl_oxide_vendored::jxl_color::tf::pq::linear_to_pq_x86_64_fma(vy, intensity_target);
        let vy_mapped = crate::jxl_oxide_vendored::jxl_color::tf::rec2408::rec2408_eetf_x86_64_fma(
            vy_pq,
            intensity_target,
            from_luminance_range,
            to_luminance_range,
        );
        let vy_mapped = crate::jxl_oxide_vendored::jxl_color::tf::pq::pq_to_linear_x86_64_fma(vy_mapped, intensity_target);
...
unsafe fn tone_map_x86_64_sse2(
    ...
    for ((r, g), b) in (&mut r).zip(&mut g).zip(&mut b) {
        ...
        let vy_pq = crate::jxl_oxide_vendored::jxl_color::tf::pq::linear_to_pq_x86_64_sse2(vy, intensity_target);
        let vy_mapped = crate::jxl_oxide_vendored::jxl_color::tf::rec2408::rec2408_eetf_x86_64_sse2(
            vy_pq,
            intensity_target,
            from_luminance_range,
            to_luminance_range,
        );
        let vy_mapped = crate::jxl_oxide_vendored::jxl_color::tf::pq::pq_to_linear_x86_64_sse2(vy_mapped, intensity_target);
        let is_small = _mm_cmplt_ps(_mm_and_ps(vy, absmask), _mm_set1_ps(1e-7));
        let vy = _mm_or_ps(
...
unsafe fn tone_map_aarch64_neon(
    ...
    for ((r, g), b) in (&mut r).zip(&mut g).zip(&mut b) {
        ...
        let vy_pq = crate::jxl_oxide_vendored::jxl_color::tf::pq::linear_to_pq_aarch64_neon(vy, intensity_target);
        let vy_mapped = crate::jxl_oxide_vendored::jxl_color::tf::rec2408::rec2408_eetf_aarch64_neon(
            vy_pq,
            intensity_target,
            from_luminance_range,
            to_luminance_range,
        );
        let vy_mapped = crate::jxl_oxide_vendored::jxl_color::tf::pq::pq_to_linear_aarch64_neon(vy_mapped, intensity_target);
