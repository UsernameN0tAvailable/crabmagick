...
pub fn linear_to_bt709(samples: &mut [f32]) {
        ...
        if std::arch::is_aarch64_feature_detected!("neon") {
            ...
            for chunk in &mut it {
                unsafe {
                    ...
                    let exp = crate::jxl_oxide_vendored::jxl_color::fastmath::fast_powf_aarch64_neon(v, 0.45);
        ...
        if std::arch::is_x86_feature_detected!("avx2") && std::arch::is_x86_feature_detected!("fma")
        {
            ...
            for chunk in &mut it {
                unsafe {
                    ...
                    let exp = crate::jxl_oxide_vendored::jxl_color::fastmath::fast_powf_x86_64_sse2(
                        v,
    ...
    for x in samples {
        ...
        *x = if a <= 0.018 {
            ...
            crate::jxl_oxide_vendored::jxl_color::fastmath::fast_powf_generic(a, 0.45).mul_add(1.099, -0.099)
        };
    }
}
...
unsafe fn linear_to_bt709_x86_64_avx2(samples: &mut [f32]) -> &mut [f32] {
    ...
    for chunk in &mut it {
        ...
        let exp = crate::jxl_oxide_vendored::jxl_color::fastmath::fast_powf_x86_64_avx2(v, _mm256_set1_ps(0.45));
    ...
    let exp = crate::jxl_oxide_vendored::jxl_color::fastmath::fast_powf_x86_64_fma(v, _mm_set1_ps(0.45));
...
pub fn bt709_to_linear(samples: &mut [f32]) {
    for x in samples {
        ...
        *x = if a <= 0.081 {
            ...
            crate::jxl_oxide_vendored::jxl_color::fastmath::fast_powf_generic(a.mul_add(1.0 / 1.099, 0.099 / 1.099), 1.0 / 0.45)
        };
    }
}
