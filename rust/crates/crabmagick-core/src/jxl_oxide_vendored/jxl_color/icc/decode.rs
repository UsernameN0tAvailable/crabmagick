...
use crate::jxl_oxide_vendored::jxl_color::{Error, Result};
...
pub fn read_icc(bitstream: &mut Bitstream) -> Result<Vec<u8>> {
    ...
    if enc_size > (1 << 28) {
        ...
        return Err(
            crate::jxl_oxide_vendored::jxl_bitstream::Error::ProfileConformance("Too large encoded ICC profile").into(),
        );
    }

    let mut decoder = crate::jxl_oxide_vendored::jxl_coding::Decoder::parse(bitstream, 41)?;
    ...
    if output_size > (1 << 28) {
        return Err(crate::jxl_oxide_vendored::jxl_bitstream::Error::ProfileConformance("ICC output_size too large").into());
    }
