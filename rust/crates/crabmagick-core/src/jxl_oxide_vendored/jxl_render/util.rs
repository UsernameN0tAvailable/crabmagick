...
use crate::jxl_oxide_vendored::jxl_frame::{
    Frame, FrameHeader,
    data::{LfGlobal, LfGroup},
    filter::{EdgePreservingFilter, EpfParams},
    header::FrameType,
};
use crate::jxl_oxide_vendored::jxl_grid::{AlignedGrid, MutableSubgrid};
use crate::jxl_oxide_vendored::jxl_image::ImageHeader;
use crate::jxl_oxide_vendored::jxl_modular::{ChannelShift, Sample, image::TransformedModularSubimage};
use crate::jxl_oxide_vendored::jxl_threadpool::JxlThreadPool;

use crate::jxl_oxide_vendored::jxl_render::{
        ...
        tracing::trace_span!("XYB to target colorspace").in_scope(|| -> Result<_> {
            ...
            let transform = ColorTransform::new(
                &ColorEncodingWithProfile::new(EnumColourEncoding::xyb(
                    crate::jxl_oxide_vendored::jxl_color::RenderingIntent::Perceptual,
                )),
                &ColorEncodingWithProfile::new(encoding.clone()),
                &metadata.opsin_inverse_matrix,
                &metadata.tone_mapping,
                &crate::jxl_oxide_vendored::jxl_color::NullCms,
            )
            .unwrap();
...
impl<S: Default + Clone> PaddedGrid<S> {
    pub fn with_alloc_tracker(
        ...
        height: usize,
        padding: usize,
        tracker: Option<&crate::jxl_oxide_vendored::jxl_grid::AllocTracker>,
    ) -> Result<Self> {
        Ok(Self {
