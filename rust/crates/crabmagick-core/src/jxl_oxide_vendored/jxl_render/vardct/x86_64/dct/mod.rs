...
use crate::jxl_oxide_vendored::jxl_grid::{MutableSubgrid, SimdVector};
...
///
/// Has the same signature as [`dct_2d_x86_64_sse2`] so it can be used directly and
/// as an `unsafe fn` pointer by `transform_varblocks_inner`.
pub(crate) fn dct_2d_x86_64(io: &mut MutableSubgrid<'_>, direction: DctDirection) {
    if is_x86_feature_detected!("avx512f") {
        // SAFETY: avx512f (and, on every real CPU, its implied avx2/fma) is present.
        return unsafe { avx512::dct_2d_avx512(io, direction) };
    }
    if is_x86_feature_detected!("avx2") && is_x86_feature_detected!("fma") {
        // SAFETY: avx2 and fma are present.
        return unsafe { avx2::dct_2d_avx2(io, direction) };
    }
    dct_2d_x86_64_sse2(io, direction)
}
