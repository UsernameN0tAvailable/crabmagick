use crate::jxl_oxide_vendored::jxl_frame::Result;
use crate::jxl_oxide_vendored::jxl_bitstream::{Bitstream, U};
use crate::jxl_oxide_vendored::jxl_image::{BitDepth, Extensions, ImageHeader, SizeHeader};
use crate::jxl_oxide_vendored::jxl_oxide_common::{Bundle, Name, define_bundle};

define_bundle! {
    /// Frame header.
    #[derive(Debug)]
    pub struct FrameHeader ctx(headers: &ImageHeader) error(crate::jxl_oxide_vendored::jxl_frame::Error) {
    ...
    #[derive(Debug)]
    pub struct Passes error(crate::jxl_oxide_vendored::jxl_frame::Error) {
    ...
    #[derive(Debug)]
    pub struct BlendingInfo ctx(context: (bool, Option<BlendMode>, CanvasSizeParams<'_>)) error(crate::jxl_oxide_vendored::jxl_frame::Error) {
    ...
    #[derive(Debug)]
    pub struct RestorationFilter ctx(encoding: Encoding) error(crate::jxl_oxide_vendored::jxl_frame::Error) {
        all_default: ty(Bool) default(true),
        pub gab: ty(Bundle(crate::jxl_oxide_vendored::jxl_frame::filter::Gabor)) cond(!all_default),
        pub epf: ty(Bundle(crate::jxl_oxide_vendored::jxl_frame::filter::EdgePreservingFilter)) cond(!all_default),
        pub extensions: ty(Bundle(Extensions)) cond(!all_default),
    }
}
...
impl<Ctx> Bundle<Ctx> for BlendMode {
    ...
    fn parse(bitstream: &mut Bitstream, _ctx: Ctx) -> Result<Self> {
        Ok(match bitstream.read_u32(0, 1, 2, 3 + U(2))? {
            ...
            4 => Self::Mul,
            value => {
                return Err(crate::jxl_oxide_vendored::jxl_bitstream::Error::InvalidEnum {
                    name: "BlendMode",
                    value,
                }
                .into());
