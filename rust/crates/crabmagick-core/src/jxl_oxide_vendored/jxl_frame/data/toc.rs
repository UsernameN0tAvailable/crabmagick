...
impl Bundle<&crate::jxl_oxide_vendored::jxl_frame::FrameHeader> for Toc {
    type Error = crate::jxl_oxide_vendored::jxl_frame::Error;

    fn parse(bitstream: &mut Bitstream, ctx: &crate::jxl_oxide_vendored::jxl_frame::FrameHeader) -> Result<Self> {
        ...
        if entry_count > 65536 {
            return Err(crate::jxl_oxide_vendored::jxl_bitstream::Error::ValidationFailed("Too many TOC entries").into());
        }

        let permutated_toc = bitstream.read_bool()?;
        let permutation = if permutated_toc {
            let mut decoder = crate::jxl_oxide_vendored::jxl_coding::Decoder::parse(bitstream, 8)?;
            decoder.begin(bitstream)?;
            let permutation =
                crate::jxl_oxide_vendored::jxl_coding::read_permutation(bitstream, &mut decoder, entry_count, 0)?;
            decoder.finalize()?;
            permutation
        } else {
            Vec::new()
