...
use crate::jxl_oxide_vendored::jxl_grid::{AllocHandle, AllocTracker};
use crate::jxl_oxide_vendored::jxl_image::ImageHeader;
use crate::jxl_oxide_vendored::jxl_oxide_common::Bundle;
...
use crate::jxl_oxide_vendored::jxl_modular::Sample;
use crate::jxl_oxide_vendored::jxl_modular::{MaConfig, image::TransformedModularSubimage};
use crate::jxl_oxide_vendored::jxl_threadpool::JxlThreadPool;

use crate::jxl_oxide_vendored::jxl_frame::data::*;
...
impl Bundle<FrameContext<'_>> for Frame {
    ...
    fn parse(bitstream: &mut Bitstream, ctx: FrameContext) -> Result<Self> {
        ...
        if width > (1 << 30) {
            ...
            return Err(crate::jxl_oxide_vendored::jxl_bitstream::Error::ProfileConformance("frame width too large").into());
        }
        if height > (1 << 30) {
            tracing::error!(width, "Frame height too large; limit is 2^30");
            return Err(crate::jxl_oxide_vendored::jxl_bitstream::Error::ProfileConformance("frame height too large").into());
        }
        if (width * height) > (1 << 40) {
            ...
            return Err(crate::jxl_oxide_vendored::jxl_bitstream::Error::ProfileConformance("frame area too large").into());
        }
        ...
        for blending_info in std::iter::once(&header.blending_info).chain(&header.ec_blending_info)
        {
            if blending_info.mode.use_alpha() && has_extra {
                ...
                let Some(alpha_ec_info) = image_header.metadata.ec_info.get(alpha_idx) else {
                    ...
                    return Err(crate::jxl_oxide_vendored::jxl_bitstream::Error::ValidationFailed(
        ...
        if header.flags.use_lf_frame() && header.lf_level >= 4 {
            return Err(crate::jxl_oxide_vendored::jxl_bitstream::Error::ValidationFailed("lf_level out of range").into());
        }
            ...
            .zip(image_header.metadata.ec_info.iter())
        {
            ...
            if actual_dim_shift > 7 + header.group_size_shift {
                return Err(crate::jxl_oxide_vendored::jxl_bitstream::Error::ValidationFailed("dim_shift too large").into());
            }
        }
        ...
        if self.toc.is_single_entry() {
            ...
            if offset == 0 {
                ...
                let lf_group = self
                    ...
                    .ok_or(
                        crate::jxl_oxide_vendored::jxl_bitstream::Error::Io(std::io::ErrorKind::UnexpectedEof.into()).into(),
                    )
                    .and_then(|x| x);
