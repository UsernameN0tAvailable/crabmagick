...
use crate::jxl_oxide_vendored::jxl_coding::{CodingResult, Error};
...
impl Histogram {
    /// Decodes a single ANS symbol, threading `state` and consuming from `bitstream`.
    ///
    /// Note on parallelism: JPEG XL's ANS is a **single-state** rANS. The next symbol's table
    /// index (`state & 0xfff`) and the shared bitstream refill both depend on the fully updated
    /// state produced here, so successive decodes form a strict data-dependency chain and cannot
    /// run as independent in-flight states (that pattern only applies to *multi-state* interleaved
    /// rANS such as Giesen's rANS or zstd/FSE, which JXL does not use). [`read_symbols_batched`]
    /// therefore unrolls this operation 4× to cut per-call overhead and expose ILP to the
    ...
    /// Decodes a single ANS symbol. Thin wrapper over [`decode_symbol_once`] kept for the
    /// single-symbol call sites; behaviour is byte-for-byte identical to the original.
    #[inline(always)]
    pub fn read_symbol(&self, bitstream: &mut Bitstream, state: &mut u32) -> CodingResult<u32> {
        self.decode_symbol_once(bitstream, state)
    }

    /// Decodes `out.len()` symbols from this (single) distribution, threading `state` and the
    /// bitstream exactly as repeated [`read_symbol`] calls would.
    ///
    /// The hot loop is unrolled 4× (`chunks_exact_mut(4)`), with a scalar cleanup loop handling
    /// the remainder. Because JXL ANS is single-state (see [`decode_symbol_once`]) the four decodes
    /// within an iteration remain sequential; the unrolling reduces loop/branch bookkeeping and lets
    /// the CPU overlap independent micro-ops across the four bodies where the state chain allows.
    #[inline]
    pub fn read_symbols_batched(
        ...
        for chunk in &mut chunks {
            chunk[0] = self.decode_symbol_once(bitstream, state)?;
            chunk[1] = self.decode_symbol_once(bitstream, state)?;
            chunk[2] = self.decode_symbol_once(bitstream, state)?;
            chunk[3] = self.decode_symbol_once(bitstream, state)?;
        }
        for slot in chunks.into_remainder() {
            *slot = self.decode_symbol_once(bitstream, state)?;
        }
        Ok(())
    }
