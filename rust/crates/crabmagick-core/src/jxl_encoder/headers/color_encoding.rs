...
impl ColorEncoding {
    ...
    pub fn write(&self, writer: &mut BitWriter) -> Result<()> {
        ...
        crate::jxl_encoder::trace::debug_eprintln!("CENC [bit {}]: color_encoding done", writer.bits_written());
...
mod tests {
    ...
    fn test_roundtrip_custom_white_point_d50() {
        ...
        let encoded = crate::jxl_encoder::LosslessConfig::new()
            .encode_request(width, height, crate::jxl_encoder::PixelLayout::Rgb8)
            .with_color_encoding(ce)
            .encode(&pixels)
            .expect("encoding with custom white point should succeed");

        // Decode with jxl-rs (primary decoder)
        let decoded = crate::jxl_encoder::test_helpers::decode_with_jxl_rs(&encoded)
