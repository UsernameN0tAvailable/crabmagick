...
use crate::jxl_encoder::BLOCK_SIZE;
use crate::jxl_encoder::bit_writer::BitWriter;
use crate::jxl_encoder::container::{wrap_in_container, wrap_in_container_jxlp};
...
use crate::jxl_encoder::headers::color_encoding::ColorEncoding;
use crate::jxl_encoder::headers::file_header::{BitDepth, FileHeader, ImageMetadata};
use crate::jxl_encoder::headers::frame_header::{Encoding, FrameHeader};
use crate::jxl_encoder::parallel::parallel_map;
use crate::jxl_encoder::parallel::parallel_map_result;
use crate::jxl_encoder::vardct::ac_context;
use crate::jxl_encoder::vardct::ac_group::{collect_ac_coefficients_dct8_into, predict_from_top_and_left};
use crate::jxl_encoder::vardct::ac_strategy::AcStrategyMap;
use crate::jxl_encoder::vardct::common::*;
use crate::jxl_encoder::vardct::dc_coding::{
    NUM_DC_CONTEXTS, collect_ac_metadata_tokens_jpeg_dct8, collect_dc_tokens_region,
};
use crate::jxl_encoder::vardct::frame::{
...
pub fn encode_jpeg_to_jxl_sequential(jpeg: &JpegData) -> Result<Vec<u8>> {
    crate::jxl_encoder::parallel::with_sequential_maps(|| encode_jpeg_to_jxl(jpeg))
}
...
fn encode_jpeg_to_jxl_inner(jpeg: &JpegData) -> Result<(Vec<u8>, usize)> {
    ...
    if num_components != 3 && num_components != 1 {
        return Err(crate::jxl_encoder::error::Error::InvalidInput(format!(
    ...
    let (dc_code, ac_code) = if crate::jxl_encoder::parallel::sequential_maps_forced() {
        (
    ...
    if let Some(ref icc) = icc_profile {
        crate::jxl_encoder::icc::write_icc(icc, &mut writer)?;
    }
...
pub fn encode_jpeg_to_jxl_container(jpeg: &JpegData) -> Result<Vec<u8>> {
    ...
    if jpeg.is_progressive {
        // Progressive scan ordering is not serialized into JBRD, but zenjpeg still
...
pub fn encode_jpeg_to_jxl_container_sequential(jpeg: &JpegData) -> Result<Vec<u8>> {
    crate::jxl_encoder::parallel::with_sequential_maps(|| encode_jpeg_to_jxl_container(jpeg))
}
...
fn build_jpeg_file_header(width: usize, height: usize, is_gray: bool) -> FileHeader {
    let color_encoding = if is_gray {
        ...
        ColorEncoding {
            rendering_intent: crate::jxl_encoder::headers::color_encoding::RenderingIntent::Relative,
            ..ColorEncoding::gray()
        }
    } else {
...
fn write_dc_global_jpeg(
    ...
    crate::jxl_encoder::vardct::context_tree::write_block_context_map(writer)?;
    ...
    crate::jxl_encoder::vardct::context_tree::write_context_tree(num_dc_groups, writer)?;
...
fn write_raw_quant_table_modular(qtables: &[i32], writer: &mut BitWriter) -> Result<()> {
    ...
    use crate::jxl_encoder::modular::section::collect_all_residuals;
    ...
    let (tree_depths, tree_codes) =
        crate::jxl_encoder::modular::encode::write_tree_histogram_for_gradient(writer)?;
    // Write tree tokens (single leaf: property=0, predictor=Gradient, offset=0, mul=1)
    crate::jxl_encoder::modular::encode::write_gradient_tree_tokens(writer, &tree_depths, &tree_codes)?;
    ...
    let (tokens, code) = crate::jxl_encoder::modular::encode::build_ans_modular_code(&residuals);
    crate::jxl_encoder::modular::encode::write_ans_modular_header(writer, &code)?;

    // Write data tokens
    crate::jxl_encoder::modular::encode::write_ans_modular_tokens(writer, &tokens, &code)?;
...
#[cfg(not(test))]
use crate::jxl_encoder::f16::write_f16;
#[cfg(test)]
use crate::jxl_encoder::f16::{f32_to_f16_bits, write_f16};
...
mod tests {
    ...
    fn test_encode_real_jpeg() {
        ...
        crate::jxl_encoder::test_helpers::save_test_output("jpeg-reencoding", "landscape1.jxl", &jxl);
    }

    #[test]
    fn test_encode_420_jpeg() {
        let path =
            crate::jxl_encoder::test_helpers::output_dir_for("jpeg-reencoding", "").join("test128_420.jpg");
        let data = std::fs::read(&path).expect("failed to read test JPEG");
        let jpeg = super::super::parse::read_jpeg(&data).expect("failed to parse JPEG");
