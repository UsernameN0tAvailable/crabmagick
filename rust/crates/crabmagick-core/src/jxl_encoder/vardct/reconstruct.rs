...
pub(crate) fn reconstruct_xyb(
    ...
    let mut output = core::array::from_fn(|_| crate::jxl_encoder_simd::vec_f32_dirty(num_pixels));
...
pub(crate) fn reconstruct_xyb_into(
    ...
    {
        use crate::jxl_encoder_simd::SimdToken;
        if let Some(token) = crate::jxl_encoder_simd::X64V3Token::summon() {
    ...
    {
        use crate::jxl_encoder_simd::SimdToken;
        if let Some(token) = crate::jxl_encoder_simd::NeonToken::summon() {
    ...
    {
        use crate::jxl_encoder_simd::SimdToken;
        if let Some(token) = crate::jxl_encoder_simd::Wasm128Token::summon() {
...
fn reconstruct_xyb_avx2(
    output: &mut [Vec<f32>; 3],
    _token: crate::jxl_encoder_simd::X64V3Token,
    quant_dc: &[Vec<Vec<i16>>; 3],
    quant_ac: &[Vec<Vec<[i32; DCT_BLOCK_SIZE]>>; 3],
    params: &DistanceParams,
    quant_field: &[u8],
...
fn reconstruct_xyb_neon(
    output: &mut [Vec<f32>; 3],
    _token: crate::jxl_encoder_simd::NeonToken,
    quant_dc: &[Vec<Vec<i16>>; 3],
    quant_ac: &[Vec<Vec<[i32; DCT_BLOCK_SIZE]>>; 3],
    params: &DistanceParams,
    quant_field: &[u8],
...
fn reconstruct_xyb_wasm128(
    output: &mut [Vec<f32>; 3],
    _token: crate::jxl_encoder_simd::Wasm128Token,
    quant_dc: &[Vec<Vec<i16>>; 3],
    quant_ac: &[Vec<Vec<[i32; DCT_BLOCK_SIZE]>>; 3],
    params: &DistanceParams,
    quant_field: &[u8],
...
fn reconstruct_xyb_impl_into(
    ...
    let x_qm_mul = crate::jxl_encoder_simd::fast_powf(1.25, params.x_qm_scale as f32 - 2.0);
    let b_qm_mul = crate::jxl_encoder_simd::fast_powf(1.25, params.b_qm_scale as f32 - 2.0);
...
fn reconstruct_xyb_first_block_into(
    ...
    if raw_strategy == RAW_STRATEGY_DCT8 {
        ...
        crate::jxl_encoder_simd::dequant_block_dct8(
            &quant_ac[0][by][bx],
            &quant_ac[1][by][bx],
            &quant_ac[2][by][bx],
            weights_x,
...
pub(crate) fn gab_smooth_with_scratch(
        ...
        macro_rules! gab_smooth_plane {
            ($p:expr) => {{
                GAB_SCRATCH.with(|s| {
                    ...
                    crate::jxl_encoder_simd::gab_smooth_channel($p, &mut s[..n], width, height, w_center, w1, w2);
                });
            }};
        }
    ...
    for plane in planes.iter_mut() {
        crate::jxl_encoder_simd::gab_smooth_channel(plane, scratch, width, height, w_center, w1, w2);
    }
}

pub(crate) fn gab_smooth(planes: &mut [Vec<f32>; 3], width: usize, height: usize) {
    let trace = crate::jxl_encoder::bench_trace_enabled();
...
pub(crate) fn xyb_to_linear_rgb(
    ...
    crate::jxl_encoder_simd::xyb_to_linear_rgb_batch(xyb_x, xyb_y, xyb_b, &mut linear_rgb, num_pixels);
    linear_rgb
}
...
pub(crate) fn xyb_to_linear_rgb_planar(
    ...
    crate::jxl_encoder_simd::xyb_to_linear_rgb_planar(xyb_x, xyb_y, xyb_b, out_r, out_g, out_b, num_pixels);
}
...
mod tests {
    ...
    fn test_xyb_to_linear_rgb_roundtrip() {
        use crate::jxl_encoder::color::xyb::linear_rgb_to_xyb;
