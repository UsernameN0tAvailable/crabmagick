...
///
/// Like `crate::jxl_encoder_simd::pad_plane` but writes into an existing buffer to avoid allocation.
...
pub(crate) fn apply_epf(
    ...
    if epf_iters >= 3 {
        ...
        let padded: [Vec<f32>; 3] =
            core::array::from_fn(|c| crate::jxl_encoder_simd::pad_plane(&planes[c], width, height, pad));
    ...
    if epf_iters >= 1 {
        let pad = 2;
        let in_stride = width + 2 * pad;
        let padded_x = crate::jxl_encoder_simd::pad_plane(&planes[0], width, height, pad);
        let padded_y = crate::jxl_encoder_simd::pad_plane(&planes[1], width, height, pad);
        let padded_b = crate::jxl_encoder_simd::pad_plane(&planes[2], width, height, pad);
        let mut out_x = crate::jxl_encoder_simd::vec_f32_dirty(n);
        let mut out_y = crate::jxl_encoder_simd::vec_f32_dirty(n);
        let mut out_b = crate::jxl_encoder_simd::vec_f32_dirty(n);
        epf_simd_strip_parallel(
            crate::jxl_encoder_simd::epf_step1,
            &padded_x,
            &padded_y,
            &padded_b,
            &mut out_x,
    ...
    if epf_iters >= 2 {
        ...
        let mut out_b = crate::jxl_encoder_simd::vec_f32_dirty(n);
        epf_simd_strip_parallel(
            crate::jxl_encoder_simd::epf_step2,
            &padded_x,
            &padded_y,
            &padded_b,
            &mut out_x,
...
fn compute_block_l2_errors(
    ...
    crate::jxl_encoder_simd::compute_block_l2_errors(original, reconstructed, mask1x1, xsize_blocks, ysize_blocks)
}
...
pub(crate) fn compute_epf_sharpness(
    ...
    let c3 = c3clamp.max(crate::jxl_encoder_simd::fast_powf(c3base, clamped_d));
