...
//!
//! ```ignore
//! use crate::jxl_encoder::trace::trace_write;
//!
...
macro_rules! trace_write {
    ($writer:expr, $n_bits:expr, $value:expr, $field:expr) => {{
        ...
        $crate::jxl_encoder::trace::trace_write_impl(pos, $n_bits, $value as u64, $field, None);
        result
    }};
    ($writer:expr, $n_bits:expr, $value:expr, $field:expr, $desc:expr) => {{
        let pos = $writer.bits_written();
        let result = $writer.write($n_bits, $value as u64);
        $crate::jxl_encoder::trace::trace_write_impl(pos, $n_bits, $value as u64, $field, Some($desc));
        result
    }};
}
...
macro_rules! trace_section {
    (begin $name:expr, $writer:expr) => {
        $crate::jxl_encoder::trace::push_section($name, $writer.bits_written())
    };
    (end $name:expr, $writer:expr) => {
        $crate::jxl_encoder::trace::pop_section($name, $writer.bits_written())
    };
}
...
macro_rules! trace_note {
    ($writer:expr, $($arg:tt)*) => {
        $crate::jxl_encoder::trace::trace_line(&format!("[{:6}] NOTE: {}", $writer.bits_written(), format!($($arg)*)))
    };
}
...
macro_rules! trace_bytes {
    ($writer:expr, $bytes:expr, $field:expr) => {{
        ...
        $crate::jxl_encoder::trace::trace_line(&format!(
            "[{:6}] {}: [{} bytes] {:02x?}",
            pos,
            $field,
            data.len(),
