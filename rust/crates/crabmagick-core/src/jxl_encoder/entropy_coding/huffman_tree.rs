...
pub fn store_meta_huffman_tree(
    ...
    crate::jxl_encoder::trace::debug_eprintln!("  META: final bitacc={} (should be 32)", _bitacc);
...
pub fn store_compressed_tree(
    ...
    crate::jxl_encoder::trace::debug_eprintln!("  TREE: meta_codes = {:?}", &code_length_codes[..5]);
    crate::jxl_encoder::trace::debug_eprintln!("  TREE: meta_depths = {:?}", &code_length_depths[..5]);
...
pub fn store_huffman_tree(depths: &[u8], writer: &mut BitWriter) -> Result<()> {
    ...
    crate::jxl_encoder::trace::debug_eprintln!("STORE_HUFF: code_length_histogram={:?}", histogram);
...
pub fn build_and_store_huffman_tree(
    ...
    if count <= 4 {
        // Simple Huffman code
        crate::jxl_encoder::trace::debug_eprintln!("HUFFMAN_BUILD: using simple code for {} symbols", count);
        store_simple_huffman_tree(&depths, &mut s4, count, max_bits, writer)?;
    } else {
        // Full code length table
        crate::jxl_encoder::trace::debug_eprintln!("HUFFMAN_BUILD: using full table for {} symbols", count);
