...
use crate::jxl_encoder::GROUP_DIM;
use crate::jxl_encoder::bit_writer::BitWriter;
use crate::jxl_encoder::entropy_coding::lz77::Lz77Method;
use crate::jxl_encoder::error::Result;
use crate::jxl_encoder::headers::ColorEncoding;
use crate::jxl_encoder::headers::frame_header::{BlendMode, FrameCrop, FrameHeader};
...
pub struct FrameEncoderOptions {
    ...
    pub encoder_mode: crate::jxl_encoder::api::EncoderMode,
    /// Effort profile with all effort-derived parameters.
    pub profile: crate::jxl_encoder::effort::EffortProfile,
...
impl Default for FrameEncoderOptions {
    fn default() -> Self {
        Self {
            ...
            lossy_palette: false,
            encoder_mode: crate::jxl_encoder::api::EncoderMode::Reference,
            profile: crate::jxl_encoder::effort::EffortProfile::lossless(7, crate::jxl_encoder::api::EncoderMode::Reference),
...
impl FrameEncoder {
    ...
    pub(crate) fn encode_modular_with_patches(
        ...
        patches: Option<&crate::jxl_encoder::vardct::patches::PatchesData>,
    ) -> Result<()> {
        if patches.is_none() {
        ...
        {
            use crate::jxl_encoder::headers::frame_header::PATCHES_FLAG;
        ...
        if num_groups == 1 {
            ...
            crate::jxl_encoder::vardct::patches::encode_patches_section(
                patches,
    ...
    pub fn encode_modular(
        ...
        if num_groups == 1 {
            ...
            crate::jxl_encoder::trace::debug_eprintln!("FRAME_ENCODER: section_size = {} bytes", section_size);
    ...
    fn encode_modular_multi_group_inner(
        ...
        let pass_group_data: Vec<Vec<u8>> =
            crate::jxl_encoder::parallel::parallel_map_result(num_groups * num_passes, |flat_idx| {
    ...
    fn encode_modular_multi_group_lossy_palette(
        ...
        use crate::jxl_encoder::entropy_coding::encode::{build_entropy_code_ans, write_tokens_ans};
        ...
        enum EntropyState {
            ...
            Ans {
                code: crate::jxl_encoder::entropy_coding::encode::OwnedAnsEntropyCode,
            },
        }
        ...
        let pass_group_data: Vec<Vec<u8>> =
            crate::jxl_encoder::parallel::parallel_map_result(num_groups, |g| {
    ...
    fn encode_modular_multi_group_squeeze_with_tree(
        ...
        let lf_group_sub_images: Vec<ModularImage> =
            crate::jxl_encoder::parallel::parallel_map(num_lf_groups, |lg| {
                let lg_x = lg % num_lf_groups_x;
                let lg_y = lg / num_lf_groups_x;
        ...
        let pass_group_sub_images: Vec<ModularImage> =
            crate::jxl_encoder::parallel::parallel_map(num_groups, |g| {
                let gx = g % num_groups_x;
                let gy = g / num_groups_x;
        ...
        if let Some(tokens) = pass_group_tokens.first() {
            if !tokens.is_empty() {
                ...
                for (val, cnt) in entries.iter().take(10) {
                    crate::jxl_encoder::trace::debug_eprintln!("  val={}: {}", val, cnt);
                }
            }
        }
        ...
        let lz77_method = if palette_result.is_some() {
            crate::jxl_encoder::entropy_coding::lz77::Lz77Method::Rle
        } else {
            self.options.lz77_method
        };
        let lz77_params = if use_lz77 {
            use crate::jxl_encoder::entropy_coding::lz77::apply_lz77;
            ...
            if has_lz77 {
                let mut params = crate::jxl_encoder::entropy_coding::lz77::Lz77Params::new(num_contexts, false);
                params.enabled = true;
                Some(params)
            } else {
                None
        ...
        if ans_num_contexts > 1 {
            crate::jxl_encoder::entropy_coding::lz77::write_lz77_header(
                lz77_params.as_ref(),
                &mut lf_global_writer,
            )?;
        ...
        let pass_group_data: Vec<Vec<u8>> = crate::jxl_encoder::parallel::parallel_map_result(
            num_groups,
            |g| {
    ...
    fn write_toc_multi(&self, writer: &mut BitWriter, section_sizes: &[usize]) -> Result<()> {
        crate::jxl_encoder::trace::debug_eprintln!("TOC [bit {}]: Writing permuted = 0", writer.bits_written());
        // permuted = false
        writer.write(1, 0)?;
        ...
        crate::jxl_encoder::trace::debug_eprintln!("TOC [bit {}]: After TOC entries", writer.bits_written());
...
mod tests {
    ...
    fn test_encode_multi_group_image() {
        ...
        crate::jxl_encoder::trace::debug_eprintln!("Multi-group modular: {} bytes", bytes.len());
