...
use crate::jxl_encode::{LosslessConfig, LossyConfig, PixelLayout, ProgressiveMode};

mod tests {
    use crate::jxl_encode::{LosslessConfig, LossyConfig, PixelLayout};
...
mod gray_tests {
    use crate::jxl_encode::{LosslessConfig, PixelLayout};
...
mod corpus_tests {
    ...
    fn corpus_path_string() -> String {
        crate::jxl_encode::test_helpers::corpus_dir()
            .to_string_lossy()
            .into_owned()
    }
...
mod decoder_validation {
    ...
    fn djxl_path_string() -> String {
        crate::jxl_encode::test_helpers::djxl_path()
    }

    /// Validates lossless roundtrip: encode -> decode -> compare pixels exactly.
    ///
    /// Uses jxl-rs as primary decoder (per CLAUDE.md). Falls back to JXL decoder
    /// for single-group images where both work. JXL decoder has a known limitation
    ...
    fn validate_lossless_roundtrip_rgb_config(
        ...
        let jxlrs_result = crate::jxl_encode::test_helpers::decode_with_jxl_rs(&encoded);
        ...
        // Also verify with JXL decoder (secondary decoder, may fail for multi-group ANS)
        match jxl_oxide::JxlImage::builder().read(std::io::Cursor::new(&encoded)) {
            Ok(image) => match image.render_frame(0) {
                Ok(_render) => {
                    eprintln!("{}: JXL decoder decode OK (secondary)", test_name);
                }
                Err(e) => {
                    eprintln!(
                        "{}: JXL decoder render failed (non-fatal, jxl-rs succeeded): {}",
                        test_name, e
                    );
                }
            },
            Err(e) => {
                eprintln!(
                    "{}: JXL decoder read failed (non-fatal, jxl-rs succeeded): {}",
                    test_name, e
                );
            }
        }
    ...
    fn validate_lossless_roundtrip_gray(
        ...
        let image = jxl_oxide::JxlImage::builder()
            ...
            .unwrap_or_else(|e| panic!("{}: JXL decoder decode failed: {}", test_name, e));
    ...
    /// Validates that a JXL file can be decoded by both JXL decoder and djxl.
    ...
    fn validate_dual_decoder(
        ...
        // 1. Validate with JXL decoder (Rust decoder)
        ...
        let oxide_dims = match oxide_result {
            Ok(image) => {
                assert_eq!(
                    image.width(),
                    expected_width,
                    "{}: JXL decoder width mismatch",
                    test_name
                );
                assert_eq!(
                    image.height(),
                    expected_height,
                    "{}: JXL decoder height mismatch",
                    test_name
                );
                (image.width(), image.height())
            }
            Err(e) => {
                panic!("{}: JXL decoder decode failed: {:?}", test_name, e);
            }
        };
        ...
        if std::path::Path::new(&djxl).exists() {
            ...
            match Command::new(&djxl).args([&temp_jxl, &temp_png]).output() {
                Ok(output) if output.status.success() => {
                    ...
                    eprintln!(
                        "{}: PASSED dual-decoder validation (JXL decoder + djxl)",
                        test_name
                    );
                }
                Ok(output) => {
                    ...
                    {
                        eprintln!(
                            "{}: PASSED JXL decoder only (djxl missing shared libs)",
                            test_name
                        );
                    } else {
                ...
                Err(e) => {
                    eprintln!(
                        "{}: PASSED JXL decoder only (djxl not runnable: {})",
                        test_name, e
                    );
                }
            }
            ...
            eprintln!(
                "{}: PASSED JXL decoder only (djxl not available at {})",
                test_name, djxl
            );
        }
    ...
    /// Test that our encoded files can be decoded by JXL decoder
    #[test]
    fn test_decode_simple_gray() {
        ...
        // Try to decode with JXL decoder
        ...
        match decoder {
            ...
            Err(e) => {
                ...
                eprintln!();
                panic!("JXL decoder failed to decode our encoded file");
            }
        }
    }

    /// Test that our lossless RGB encoded files can be decoded by JXL decoder
    #[test]
    fn test_decode_lossless_rgb() {
        ...
        match decoder {
            ...
            Err(e) => {
                ...
                eprintln!();
                panic!("JXL decoder failed to decode lossless RGB file: {:?}", e);
            }
        }
    }
    ...
    fn test_decode_lossy_rgb() {
        ...
        match decoder {
            ...
            Err(e) => {
                ...
                eprintln!();
                panic!("JXL decoder failed to decode lossy file: {:?}", e);
            }
        }
    }
    ...
    /// Test that pngsuite RGB images can be decoded by JXL decoder
    #[test]
    fn test_decode_pngsuite_rgb() {
        crate::jxl_encode::skip_without_corpus!();
        // (corpus path resolved via crate::jxl_encode::test_helpers::corpus_dir)
        let path = format!(
        ...
        match jxl_oxide::JxlImage::builder().read(std::io::Cursor::new(&encoded)) {
            ...
            Err(e) => {
                ...
                panic!("JXL decoder failed to decode basn2c08: {:?}", e);
            }
        }
    }
    ...
    fn test_decode_lossy_multi_group() {
        ...
        // Verify decodes correctly with JXL decoder
        match jxl_oxide::JxlImage::builder().read(std::io::Cursor::new(&encoded)) {
            Ok(image) => {
            ...
            Err(e) => {
                ...
                panic!("JXL decoder failed to decode multi-group file: {:?}", e);
            }
        }
    }
    ...
    fn test_lossy_multi_dc_group_roundtrip() {
        ...
        for effort in [3, 5, 7] {
            ...
            // Verify with JXL decoder
            let image = jxl_oxide::JxlImage::builder()
                .read(std::io::Cursor::new(&encoded))
                .unwrap_or_else(|e| panic!("JXL decoder decode failed for {w}x{h} e{effort}: {e:?}"));
            assert_eq!(image.width(), w as u32);
            assert_eq!(image.height(), h as u32);
            let _render = image
                .render_frame(0)
                .unwrap_or_else(|e| panic!("JXL decoder render failed for {w}x{h} e{effort}: {e:?}"));
    ...
    fn test_lossy_wide_gamut_p3_roundtrip() {
        use crate::jxl_encode::headers::color_encoding::{ColorEncoding, Primaries, TransferFunction};
        ...
        // Decode to sRGB linear via JXL decoder
        ...
        let render = image.render_frame(0).expect("JXL decoder render failed");
        let fb = render.image_all_channels();
    ...
    // These tests validate encoded files against BOTH JXL decoder and djxl
    ...
    fn test_dual_decode_lossy() {
        ...
        // VarDCT is validated against JXL decoder only until encoder is complete
        let oxide_result = jxl_oxide::JxlImage::builder().read(std::io::Cursor::new(&encoded));
        assert!(
            oxide_result.is_ok(),
            "JXL decoder should decode lossy VarDCT: {:?}",
            oxide_result.err()
        );
        ...
        eprintln!("lossy_16x16: PASSED JXL decoder (rendered successfully)");
    }
    ...
    /// Note: VarDCT encoding is WIP - JXL decoder only
    #[test]
    fn test_dual_decode_lossy_distances() {
        ...
        for distance in [0.5, 1.0, 2.0, 4.0] {
            let encoded = LossyConfig::new(distance)
                .encode(&data, 8, 8, PixelLayout::Rgb8)
                .unwrap();
            // VarDCT validated against JXL decoder only
            let oxide_result = jxl_oxide::JxlImage::builder().read(std::io::Cursor::new(&encoded));
            assert!(
                oxide_result.is_ok(),
                "JXL decoder should decode at distance {}",
                distance
            );
            let image = oxide_result.unwrap();
            assert_eq!(image.width(), 8);
        ...
        eprintln!("lossy_distances: PASSED JXL decoder (rendered successfully)");
    }
    ...
    /// Note: VarDCT encoding is WIP - JXL decoder only for lossy
    #[test]
    fn test_dual_decode_multi_group() {
        ...
        // Also test lossy with JXL decoder only
        ...
        assert!(
            oxide_result.is_ok(),
            "JXL decoder should decode multi-group lossy"
        );
        let image = oxide_result.unwrap();
        assert_eq!(image.width(), 256);
        assert_eq!(image.height(), 256);
        eprintln!("multi_group_256x256_lossy: PASSED JXL decoder (VarDCT WIP)");
    }
    ...
    fn test_tree_learning_select_predictor() {
        ...
        for size in [64, 192, 256] {
            ...
            for effort in [6, 7, 8] {
                ...
                let decoded =
                    crate::jxl_encode::test_helpers::decode_with_jxl_rs(&encoded).unwrap_or_else(|e| {
                        panic!(
    ...
    fn test_encode_multigroup_300x300() {
        ...
        assert!(
            result.is_ok(),
            "JXL decoder should decode multi-group: {:?}",
            result.err()
        );
        let image = result.unwrap();
        assert_eq!(image.width(), 300);
        assert_eq!(image.height(), 300);
        eprintln!("Multi-group 300x300 decode: PASSED JXL decoder");
    }
    ...
    fn test_encode_multigroup_512x512() {
        ...
        assert!(
            result.is_ok(),
            "JXL decoder should decode 512x512 multi-group: {:?}",
            result.err()
        );
        let image = result.unwrap();
        assert_eq!(image.width(), 512);
        assert_eq!(image.height(), 512);
        eprintln!("Multi-group 512x512 decode: PASSED JXL decoder");
    }
    ...
    /// Debug test to understand JXL decoder output format
    #[test]
    ...
    #[test]
    #[ignore = "VarDCT lossy encoding has known JXL decoder compatibility issues"]
    ...
    #[cfg(test)]
    mod lossy_tests {
        use crate::jxl_encode::test_helpers::*;
...
mod quality_comparison_tests {
    use crate::jxl_encode::{LossyConfig, PixelLayout};
    ...
    fn libjxl_cjxl() -> String {
        crate::jxl_encode::test_helpers::cjxl_path()
    }
    ...
    /// Encode with our encoder and decode with JXL decoder
...
/// Tests that both jxl-rs and JXL decoder produce identical results
...
mod dual_decoder_butteraugli_tests {
    ...
    /// Decode JXL with JXL decoder and return RGB8 pixels
    fn decode_with_oxide(jxl_data: &[u8]) -> Result<(Vec<u8>, usize, usize), String> {
        let image = jxl_oxide::JxlImage::builder()
            .read(Cursor::new(jxl_data))
            .map_err(|e| format!("JXL decoder parse error: {:?}", e))?;

        let frame = image
            .render_frame(0)
            .map_err(|e| format!("JXL decoder render error: {:?}", e))?;
    ...
    fn decode_with_jxlrs(jxl_data: &[u8]) -> Result<(Vec<u8>, usize, usize), String> {
        ...
        let output = Command::new(crate::jxl_encode::test_helpers::jxl_cli_path())
    ...
    fn test_dual_decoder_butteraugli_sweep() {
        // Check prerequisites
        let corpus = crate::jxl_encode::test_helpers::corpus_dir();
        if !corpus.exists() {
            eprintln!("SKIP: corpus not found at {}", corpus.display());
            return;
        }
        let jxlrs_cli = crate::jxl_encode::test_helpers::jxl_cli_path();
    ...
    fn test_dual_decoder_butteraugli_quick() {
        ...
        for distance in distances {
            ...
            let (oxide_decoded, _, _) =
                decode_with_oxide(&encoded).expect("JXL decoder decode failed");
    ...
    fn test_corpus_clic_cid() {
        ...
        let corpus_path = crate::jxl_encode::test_helpers::corpus_dir();
    ...
    fn test_frymire_quality() {
        // frymire.png is stored in jxl_encode/tests/images/
...
mod tree_learning_tests {
    ...
    fn validate_tree_learning_roundtrip_rgb(
        ...
        // Decode with jxl-rs (PRIMARY decoder)
        let decoded_img = crate::jxl_encode::test_helpers::decode_with_jxl_rs(&encoded)
...
fn validate_palette_roundtrip_rgb(data: &[u8], width: usize, height: usize, test_name: &str) {
    ...
    crate::jxl_encode::test_helpers::save_test_output("palette", &format!("{test_name}.jxl"), &encoded);
...
fn test_squeeze_roundtrip_gray_16x16() {
    ...
    crate::jxl_encode::test_helpers::save_test_output("squeeze", "squeeze_gray_16x16.jxl", &bytes);

    // Decode with jxl-rs
    let decoded_img =
        crate::jxl_encode::test_helpers::decode_with_jxl_rs(&bytes).expect("jxl-rs decode failed");
...
fn test_squeeze_roundtrip_rgb_32x32() {
    ...
    crate::jxl_encode::test_helpers::save_test_output("squeeze", "squeeze_rgb_32x32.jxl", &bytes);
...
fn test_squeeze_multigroup_gray_300x300() {
    ...
    crate::jxl_encode::test_helpers::save_test_output("squeeze", "squeeze_multigroup_gray_300x300.jxl", &bytes);
...
fn test_squeeze_multigroup_rgb_300x300() {
    ...
    crate::jxl_encode::test_helpers::save_test_output("squeeze", "squeeze_multigroup_rgb_300x300.jxl", &bytes);
...
fn test_squeeze_multigroup_rgb_512x512() {
    ...
    crate::jxl_encode::test_helpers::save_test_output("squeeze", "squeeze_multigroup_rgb_512x512.jxl", &bytes);
...
fn test_rgb_lossless_djxl_sweep() {
    ...
    for w in 4..40 {
        for h in 4..40 {
            ...
            match crate::jxl_encode::test_helpers::decode_with_jxl_rs(&encoded) {
                Ok(img) => {
            ...
            let output = Command::new(crate::jxl_encode::test_helpers::djxl_path())
...
fn test_tree_learning_debug_single() {
    ...
    for (name, w, h, pixel_fn) in &cases {
        ...
        // Decode with jxl-rs
        let jxlrs_ok = match crate::jxl_encode::test_helpers::decode_with_jxl_rs(&encoded) {
            Ok(img) => {
                let decoded: Vec<u8> = img
                    .pixels
                    .iter()
...
fn test_progressive_minimal() {
    use crate::jxl_encode::test_helpers::{decode_with_djxl, decode_with_jxl_rs};
...
fn test_splines_roundtrip_jxl_rs() {
    use crate::jxl_encode::test_helpers::decode_with_jxl_rs;
    use crate::jxl_encode::{LossyConfig, PixelLayout, Spline, SplinePoint};
    ...
    crate::jxl_encode::test_helpers::save_test_output("splines", "diagonal_d1.jxl", &encoded);
...
fn test_splines_roundtrip_djxl() {
    crate::jxl_encode::skip_without_binary!(crate::jxl_encode::test_helpers::djxl_path());
    use crate::jxl_encode::test_helpers::decode_with_djxl;
    use crate::jxl_encode::{LossyConfig, PixelLayout, Spline, SplinePoint};
    ...
    crate::jxl_encode::test_helpers::save_test_output("splines", "diagonal_d1_djxl.jxl", &encoded);
