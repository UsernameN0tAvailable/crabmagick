...
/// [`EffortProfile`](crate::jxl_encode::effort::EffortProfile).
///
...
pub(super) fn compute_scaled_constants(distance: f32, bases: (f32, f32, f32)) -> (f32, f32, f32) {
    ...
    let info_loss_mul = info_loss_base * crate::jxl_encode_simd::fast_powf(ratio, K_POW_INFO_LOSS);
    let zeros_mul = zeros_base * crate::jxl_encode_simd::fast_powf(ratio, K_POW_ZEROS_MUL);
    let cost_delta = cost_delta_base * crate::jxl_encode_simd::fast_powf(ratio, K_POW_COST_DELTA);
    (info_loss_mul, cost_delta, zeros_mul)
}

use crate::jxl_encode::effort::EntropyMulTable;
...
fn estimate_entropy_full_impl(
    ...
    if raw_strategy == RAW_STRATEGY_DCT8 && use_pixel_domain {
        ...
        let mut process_channel =
            |c: usize, y_dct_ref: &[f32; 64], dct_out: Option<&mut [f32; 64]>| {
                ...
                let coeff_result = crate::jxl_encode_simd::fused_dct8_entropy(
                    xyb[c],
                    stride,
                    bx,
                    by,
                ...
                let mut channel_loss = crate::jxl_encode_simd::pixel_domain_loss(
                    pixel_error_buf,
                    mask,
                    mask_row_base,
                    mask1x1_stride,
    ...
    for (c, &cmap_factor) in cmap_factors.iter().enumerate() {
        ...
        let coeff_result = crate::jxl_encode_simd::entropy_estimate_coeffs(
            block_channels[c],
