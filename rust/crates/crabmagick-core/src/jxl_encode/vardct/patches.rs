...
pub(crate) fn find_text_like_patches(
    ...
    #[cfg(feature = "parallel")]
    let num_seeds: u32 = if !crate::jxl_encode::parallel::sequential_maps_forced() {
        use rayon::prelude::*;
...
pub(crate) fn encode_patches_section(
    ...
    if use_ans {
        ...
        crate::jxl_encode::entropy_coding::encode::write_entropy_code_ans(&code, writer)?;
        crate::jxl_encode::entropy_coding::encode::write_tokens_ans(&tokens, &code, None, writer)?;
    } else {
        ...
        crate::jxl_encode::entropy_coding::encode::write_tokens(&tokens, &ec, None, writer)?;
    }
...
pub(crate) fn subtract_patches_modular(
    image: &mut crate::jxl_encode::modular::channel::ModularImage,
    patches: &PatchesData,
    bit_depth: u32,
) {
    let max_val = ((1u32 << bit_depth) - 1) as f32;
...
pub(crate) fn encode_reference_frame_rgb(
    ...
    use crate::jxl_encode::headers::frame_header::{Encoding, FrameHeader, FrameType};
...
mod tests {
    ...
    fn test_ref_frame_value_ranges() {
        ...
        crate::jxl_encode::color::xyb::srgb_image_to_xyb(&r, &g, &b, &mut x_out, &mut y_out, &mut b_out);
    ...
    /// Use `cargo test -p crabmagick-core --lib patches::tests::test_terminal_patch_coverage -- --ignored --nocapture`
    #[test]
    #[ignore]
