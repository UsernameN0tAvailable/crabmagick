...
use crate::jxl_encode::parallel::{parallel_accumulate, parallel_map};
...
pub fn build_entropy_code_from_accumulated_ans(
    ...
    use crate::jxl_encode::entropy_coding::cluster::{
        ClusteringType, EntropyType, cluster_histograms as enhanced_cluster,
    };
    use crate::jxl_encode::entropy_coding::histogram::Histogram as EnhancedHistogram;
...
fn optimize_uint_configs_fast_from_freqs(
    ...
    use crate::jxl_encode::entropy_coding::ans::ANS_MAX_ALPHABET_SIZE;
    ...
    parallel_map(freqs_per_histo.len(), |h| {
        ...
        for &cfg in &candidates {
            ...
            for &count in &counts_buf[..capacity] {
                if count > 0 {
                    let c = count as f32;
                    entropy_cost -= c as f64 * crate::jxl_encode_simd::fast_log2f(c * inv_total) as f64;
                }
            }
...
fn estimate_context_map_cost(tokens: &[u8]) -> f64 {
    ...
    for &c in &counts {
        if c > 0 {
            let cf = c as f32;
            let p = cf * inv_total;
            cost -= p * crate::jxl_encode_simd::fast_log2f(p);
        }
    }
...
pub fn verify_histogram_serialization(code: &OwnedAnsEntropyCode, label: &str) -> Result<()> {
    use crate::jxl_encode::entropy_coding::ans_decode::{AnsHistogram, BitReader};
        ...
        if mismatch {
            #[cfg(feature = "debug-rect")]
            {
                ...
                for (k, &c) in histo.counts.iter().enumerate().take(histo.alphabet_size) {
                    if c > 0 {
                        let lc = crate::jxl_encode::entropy_coding::ans::floor_log2_ans(c as u32) + 1;
...
pub fn verify_ans_roundtrip(tokens: &[Token], code: &OwnedAnsEntropyCode) -> Result<()> {
    use crate::jxl_encode::entropy_coding::ans_decode::{AnsHistogram, AnsReader, BitReader};
        ...
        .map(|dist| {
            ...
            if let Some(single_idx) = freqs.iter().position(|&f| f == 4096) {
                let buckets = freqs
                    .iter()
                    .enumerate()
                    .map(|(i, &f)| crate::jxl_encode::entropy_coding::ans_decode::Bucket {
                        dist: f,
...
pub fn verify_ans_roundtrip_parsed(tokens: &[Token], code: &OwnedAnsEntropyCode) -> Result<()> {
    ...
    let config = code
        ...
        .first()
        .copied()
        .unwrap_or(crate::jxl_encode::entropy_coding::hybrid_uint::HybridUintConfig::default_config());
    ...
    // 8. Decode tokens
    let config = crate::jxl_encode::entropy_coding::hybrid_uint::HybridUintConfig {
        split_exponent,
    ...
    for (i, token) in tokens.iter().enumerate() {
        let (expected_encoded, _) =
            crate::jxl_encode::entropy_coding::encode::encode_token_value_with_config(token, None, &config);
