...
fn estimate_data_bits_normalized(
    ...
    {
        if *actual > 0 && *norm > 0 {
            sum += *actual as f64 * crate::jxl_encode_simd::fast_log2f(*norm as f32) as f64;
        }
    }
...
impl ANSEncodingHistogram {
    ...
    pub fn from_histogram_cached(
        ...
        let flat_data_cost = {
            let log2_alpha = crate::jxl_encode_simd::fast_log2f(alphabet_size as f32);
        ...
        if !bins.is_empty() {
            ...
            let lg2 = |v: i32| -> f64 {
                if v <= 0 {
                    ...
                    crate::jxl_encode_simd::fast_log2f(v as f32) as f64
                }
            };
...
mod tests {
    use super::*;
    use crate::jxl_encode::entropy_coding::histogram::Histogram;
