...
impl Histogram {
    ...
    pub fn shannon_entropy(&self) -> f32 {
        ...
        let entropy = crate::jxl_encode_simd::shannon_entropy_bits(&self.counts, self.total_count);
...
pub fn histogram_distance_reuse(
    ...
    let combined_entropy = crate::jxl_encode_simd::shannon_entropy_bits(combined_counts, combined_total);
...
pub fn histogram_kl_divergence(actual: &Histogram, coding: &Histogram) -> f32 {
    ...
    for (i, &count) in actual.counts.iter().enumerate() {
        if count > 0 {
            ...
            cost -= count as f32 * crate::jxl_encode_simd::fast_log2f(coding_prob);
        }
    }
