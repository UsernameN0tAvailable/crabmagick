...
//!
//! Uses JPEG encoder for reliable coefficient extraction, plus a lightweight marker
...
pub enum JpegError {
    ...
    Unsupported(String),
    #[error("JPEG encoder decode error: {0}")]
    Decode(String),
}
...
/// 2. JPEG encoder decode: extracts quantized DCT coefficients reliably
    ...
    loop {
        ...
        match marker {
            ...
            0xC2 => {
                ...
                // DCT coefficients are reconstructed from all scans by JPEG encoder.
                parse_sof_marker(data, &mut pos, &mut jpeg)?;
...
fn parse_sof_marker(data: &[u8], pos: &mut usize, jpeg: &mut JpegData) -> Result<()> {
    ...
    for (id, h, v, q) in raw {
        ...
        jpeg.components.push(JpegComponent {
            ...
            width_in_blocks: wb,
            height_in_blocks: hb,
            coeffs: Vec::new(), // Filled by JPEG encoder
        });
    }
...
fn extract_coefficients_jpeg_encoder(data: &[u8], jpeg: &mut JpegData) -> Result<()> {
    ...
    if decoded.components.len() != jpeg.components.len() {
        return Err(JpegError::Invalid(format!(
            "component count mismatch: marker scanner found {}, JPEG encoder found {}",
            jpeg.components.len(),
            decoded.components.len()
        )));
    }

    // Copy coefficients from JPEG encoder's zigzag format to our natural-order format
