...
use crate::jxl_encode::entropy_coding::huffman_tree::{
...
fn write_tree_histogram_for_predictor_impl(
    ...
    // HOWEVER, JXL decoder seems to interpret property=0 specially, treating it as a leaf.
    ...
    //   - property = 0 (works with JXL decoder as leaf marker)
...
fn write_single_leaf_tree_tokens(
    ...
    // - property = 0 (works with JXL decoder as leaf marker, though spec says should be pack_signed(-1)=1)
    // - predictor (context 1)
    // - offset = 0 (context 2)
    // - mul_log = 0 (context 3)
    ...
    crate::jxl_encode::trace::debug_eprintln!("  TREE_TOKENS: depths = {:?}", depths);
    crate::jxl_encode::trace::debug_eprintln!("  TREE_TOKENS: codes = {:?}", codes);
    ...
    crate::jxl_encode::trace::debug_eprintln!("  TREE_TOKENS [bit {}]: Done", writer.bits_written());
    Ok(())
}
