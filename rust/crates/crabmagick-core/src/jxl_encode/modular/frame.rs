...
use crate::jxl_encode::GROUP_DIM;
use crate::jxl_encode::bit_writer::BitWriter;
use crate::jxl_encode::entropy_coding::lz77::Lz77Method;
use crate::jxl_encode::error::Result;
use crate::jxl_encode::headers::ColorEncoding;
use crate::jxl_encode::headers::frame_header::{BlendMode, FrameCrop, FrameHeader};
...
impl FrameEncoder {
    ...
    pub(crate) fn encode_modular_with_patches(
        ...
        {
            use crate::jxl_encode::headers::frame_header::PATCHES_FLAG;
    ...
    fn encode_modular_multi_group_squeeze_with_tree(
        ...
        let lf_group_sub_images: Vec<ModularImage> =
            crate::jxl_encode::parallel::parallel_map(num_lf_groups, |lg| {
                let lg_x = lg % num_lf_groups_x;
                let lg_y = lg / num_lf_groups_x;
        ...
        let pass_group_sub_images: Vec<ModularImage> =
            crate::jxl_encode::parallel::parallel_map(num_groups, |g| {
                let gx = g % num_groups_x;
                let gy = g / num_groups_x;
        ...
        if let Some(tokens) = pass_group_tokens.first() {
            if !tokens.is_empty() {
                ...
                for (val, cnt) in entries.iter().take(10) {
                    crate::jxl_encode::trace::debug_eprintln!("  val={}: {}", val, cnt);
                }
            }
        }
        ...
        let lz77_method = if palette_result.is_some() {
            crate::jxl_encode::entropy_coding::lz77::Lz77Method::Rle
        } else {
            self.options.lz77_method
        };
        let lz77_params = if use_lz77 {
            use crate::jxl_encode::entropy_coding::lz77::apply_lz77;
        ...
        if ans_num_contexts > 1 {
            crate::jxl_encode::entropy_coding::lz77::write_lz77_header(
                lz77_params.as_ref(),
                &mut lf_global_writer,
            )?;
...
mod tests {
    ...
    fn test_encode_multi_group_image() {
        ...
        crate::jxl_encode::trace::debug_eprintln!("Multi-group modular: {} bytes", bytes.len());
