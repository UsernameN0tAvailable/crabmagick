...
use crate::jxl_encode::entropy_coding::encode::write_tokens_ans;
use crate::jxl_encode::error::Result;
use crate::jxl_encode::modular::channel::{Channel, ModularImage};
use crate::jxl_encode::modular::rct::{RctType, forward_rct};
            ...
            if total > 0 {
                ...
                for &count in hist.iter() {
                    if count > 0 {
                        ...
                        total_bits -= count as f64 * crate::jxl_encode_simd::fast_log2f(p as f32) as f64;
                    }
                }
            }
            hist.clear();
...
pub(crate) fn select_best_rct(image: &ModularImage, nb_rcts_to_try: u8) -> (RctType, ModularImage) {
    ...
    for (i, &rct_val) in RCT_CANDIDATES.iter().enumerate() {
        ...
        if rct_type.is_noop() {
            ...
            crate::jxl_encode::trace::debug_eprintln!("  RCT {:2}: cost={:.0}", rct_val, cost);
            if cost < best_cost {
                best_cost = cost;
                best_rct = rct_type;
...
pub fn write_modular_stream_with_tree(
    ...
    rct: bool,
    use_lz77: bool,
    lz77_method: crate::jxl_encode::entropy_coding::lz77::Lz77Method,
) -> Result<()> {
...
pub(crate) fn write_modular_stream_with_tree_dc_quant(
    ...
    use crate::jxl_encode::entropy_coding::encode::build_entropy_code_ans_with_options;
    use crate::jxl_encode::entropy_coding::encode::write_entropy_code_ans;
    use crate::jxl_encode::entropy_coding::lz77::{apply_lz77, write_lz77_header};
    ...
    if lz77_params.is_none() {
        let roundtrip_result = crate::jxl_encode::entropy_coding::encode::verify_ans_roundtrip(&tokens, &code);
...
pub fn write_modular_stream_with_squeeze_and_tree(
    ...
    let (tokens, lz77_params) = if use_lz77 {
        ...
        crate::jxl_encode::trace::debug_eprintln!("SQUEEZE LZ77: disabled");
        (tokens, None)
    };
    let ans_num_contexts = if lz77_params.is_some() {
        num_contexts + 1
...
mod tests {
    ...
    fn test_gradient_stream() {
        ...
        crate::jxl_encode::trace::debug_eprintln!("Gradient stream: {} bytes", _bytes.len());
    }
    ...
    fn test_rct_stream() {
        ...
        crate::jxl_encode::trace::debug_eprintln!("RCT stream: {} bytes", bytes.len());
    ...
    fn test_rct_type_u32_encoding() {
        use crate::jxl_encode::modular::rct::RctType;
    ...
    fn test_weighted_stream() {
        ...
        let bytes = writer.finish_with_padding();
        crate::jxl_encode::trace::debug_eprintln!("Weighted stream: {} bytes", bytes.len());
        assert!(!bytes.is_empty());
    }
    ...
    fn test_rct_weighted_stream() {
        ...
        let bytes = writer.finish_with_padding();
        crate::jxl_encode::trace::debug_eprintln!("RCT+Weighted stream: {} bytes", bytes.len());
        assert!(!bytes.is_empty());
    }
    ...
    fn test_lz77_bit_trace() {
        ...
        let _bytes = writer.finish_with_padding();
        crate::jxl_encode::trace::debug_eprintln!("LZ77 stream: {} bytes", _bytes.len());
        crate::jxl_encode::trace::debug_eprintln!("Raw bytes: {:02x?}", &_bytes[.._bytes.len().min(50)]);

        // Now let's trace through what the decoder expects:
        crate::jxl_encode::trace::debug_eprintln!("\n=== EXPECTED DECODER INTERPRETATION ===");
        crate::jxl_encode::trace::debug_eprintln!("Bit 0: dc_quant.all_default = 1");
        crate::jxl_encode::trace::debug_eprintln!("Bit 1: has_tree = 1");
        crate::jxl_encode::trace::debug_eprintln!("--- TREE HISTOGRAM (6 contexts) ---");
        crate::jxl_encode::trace::debug_eprintln!("Bit 2: lz77.enabled = 0");
        crate::jxl_encode::trace::debug_eprintln!("Bits 3-5: context_map (is_simple=1, bits_per_entry=0)");
        // ... etc
    }
    ...
    fn test_ans_roundtrip_gray() {
        ...
        let jxl_image = jxl_oxide::JxlImage::builder()
            ...
            .unwrap_or_else(|e| panic!("JXL decoder parse failed: {}", e));
        ...
        let render = jxl_image
            .render_frame(0)
            .unwrap_or_else(|e| panic!("JXL decoder render failed: {}", e));
