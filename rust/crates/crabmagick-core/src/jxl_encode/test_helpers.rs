...
macro_rules! skip_without_corpus {
    () => {
        if $crate::jxl_encode::test_helpers::try_corpus_dir().is_none() {
...
///
/// WARNING: jxl-rs has a multi-group VarDCT decoder bug (same as JXL decoder).
...
pub fn decode_with_jxl_rs(data: &[u8]) -> Result<DecodedImage> {
    ...
    let mut decoder = loop {
        match decoder
            .process(&mut input)
            .map_err(|e| crate::jxl_encode::error::Error::InvalidInput(format!("jxl-rs init error: {:?}", e)))?
        {
            ProcessingResult::Complete { result } => break result,
            ProcessingResult::NeedsMoreInput { fallback, .. } => {
                if input.is_empty() {
                    return Err(crate::jxl_encode::error::Error::InvalidInput(
    ...
    let mut decoder = loop {
        match decoder.process(&mut input).map_err(|e| {
            crate::jxl_encode::error::Error::InvalidInput(format!("jxl-rs frame error: {:?}", e))
        })? {
            ProcessingResult::Complete { result } => break result,
            ProcessingResult::NeedsMoreInput { fallback, .. } => {
                if input.is_empty() {
    ...
    let mut color_buffer = Image::<f32>::new((width * num_channels, height)).map_err(|e| {
        crate::jxl_encode::error::Error::InvalidInput(format!("jxl-rs buffer alloc error: {:?}", e))
    })?;

    let mut extra_buffers: Vec<Image<f32>> = (0..num_extra)
        .map(|_| {
            Image::<f32>::new((width, height)).map_err(|e| {
                crate::jxl_encode::error::Error::InvalidInput(format!(
    ...
    loop {
        match decoder.process(&mut input, &mut buffers).map_err(|e| {
            crate::jxl_encode::error::Error::InvalidInput(format!("jxl-rs decode error: {:?}", e))
        })? {
...
/// since both jxl-rs and JXL decoder have multi-group decoder bugs.
///
...
pub fn decode_with_djxl(data: &[u8]) -> Result<DecodedImage> {
    ...
    std::fs::write(&temp_jxl, data).map_err(|e| {
        crate::jxl_encode::error::Error::InvalidInput(format!("Failed to write temp file: {:?}", e))
    })?;
    ...
    let output = Command::new(&djxl)
        ...
        .output()
        .map_err(|e| crate::jxl_encode::error::Error::InvalidInput(format!("Failed to run djxl: {:?}", e)))?;
    ...
    let img = image::open(&temp_png).map_err(|e| {
        ...
        crate::jxl_encode::error::Error::InvalidInput(format!("Failed to load decoded PNG: {:?}", e))
    })?;
    let rgb = img.to_rgb8();
...
/// Decode JXL data using JXL decoder (SECONDARY decoder).
///
/// WARNING: JXL decoder has a multi-group VarDCT decoder bug.
/// For images >256x256, use decode_with_djxl() instead.
#[cfg(test)]
pub fn decode_with_jxl_oxide(data: &[u8]) -> Result<DecodedImage> {
    let mut image = jxl_oxide::JxlImage::builder()
        ...
        .map_err(|e| {
            crate::jxl_encode::error::Error::InvalidInput(format!("JXL decoder decode failed: {:?}", e))
        })?;
    ...
    let render = image.render_frame(0).map_err(|e| {
        crate::jxl_encode::error::Error::InvalidInput(format!("JXL decoder render failed: {:?}", e))
    })?;
...
pub fn test_lossless_roundtrip(
    ...
    let encoded = crate::jxl_encode::LosslessConfig::new()
        .encode(data, width as u32, height as u32, crate::jxl_encode::PixelLayout::Rgb8)
        .map_err(|e| crate::jxl_encode::error::Error::InvalidInput(format!("{e}")))?;
...
pub fn test_lossy_roundtrip(
    ...
    let encoded = crate::jxl_encode::LossyConfig::new(distance)
        .encode(data, width as u32, height as u32, crate::jxl_encode::PixelLayout::Rgb8)
        .map_err(|e| crate::jxl_encode::error::Error::InvalidInput(format!("{e}")))?;
