...
use crate::zen_jp2::error::{DecodingError, Result, TileError, bail};
use crate::zen_jp2::j2c::segment::MAX_BITPLANE_COUNT;
use crate::zen_jp2::math::SimdBuffer;
...
fn decode_sub_band_bitplanes(
    ...
    let dequantization_step = {
        if component_info.quantization_info.quantization_style == QuantizationStyle::NoQuantization
        {
            ...
            crate::zen_jp2::math::pow2i(r_b as i32 - exponent as i32) * (1.0 + (mantissa as f32) / 2048.0)
        }
    };
        ...
        if work.len() > 4 {
            ...
            work.into_par_iter().try_for_each_init(
                ...
                |task_ctx, code_block_idx| {
                    ...
                    Ok::<(), crate::zen_jp2::error::DecodeError>(())
                },
            )?;
...
fn apply_sign_shift(channel_data: &mut [ComponentData], component_infos: &[ComponentInfo]) {
    use crate::zen_jp2::math::{Level, dispatch, f32x8};
