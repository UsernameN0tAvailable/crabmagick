...
use crate::zen_jp2::error::{FormatError, Result, bail};
use crate::zen_jp2::j2c::ComponentData;
use crate::zen_jp2::jp2::r#box::{FILE_TYPE, JP2_SIGNATURE};
use crate::zen_jp2::jp2::cdef::ChannelDefinitionBox;
use crate::zen_jp2::jp2::cmap::{ComponentMappingBox, ComponentMappingEntry, ComponentMappingType};
use crate::zen_jp2::jp2::colr::ColorSpecificationBox;
use crate::zen_jp2::jp2::pclr::PaletteBox;
use crate::zen_jp2::reader::BitReader;
use crate::zen_jp2::{DecodeSettings, Image, resolve_alpha_and_color_space};
    ...
    while !reader.at_end() {
        ...
        match current_box.box_type {
            ...
            r#box::CONTIGUOUS_CODESTREAM => {
                parsed_codestream = Some(crate::zen_jp2::j2c::parse_raw(current_box.data, &settings)?);
            }
            _ => {}
        }
    }
