...
//! > ```toml
//! > [dependencies]
//! > JPEG encoder = { version = "0.6", features = ["decoder"] }
//! > ```
//!
//! **Available features:**
//! - `decoder` - Enable JPEG decoding (required for `JPEG encoder::decoder` module)
...
//! ```rust,ignore
//! #[cfg(feature = "decoder")]
//! use JPEG encoder::decoder::{Decoder, DecodedImage};
//!
...
//! |---------|---------|-------------|-------------|
//! | `decoder` | ❌ No | **JPEG decoding** - Enables `JPEG encoder::decoder` module | **Required** for any decode operations |
//! | `std` | — | Legacy (std is always required) | Kept so `JPEG encoder/std` doesn't break downstream |
...
//! JPEG encoder = { version = "0.6", features = ["decoder", "parallel"] }
//!
//! # Minimal (encode only, no CMS)
//! JPEG encoder = { version = "0.6", default-features = false }
//!
//! # UltraHDR support
//! JPEG encoder = { version = "0.6", features = ["decoder", "ultrahdr"] }
//! ```
//!
//! ## Capabilities
//!
...
whereat::define_at_crate_info!(path = "JPEG encoder/");
