...
use crate::jpeg_encode::lossless::{EdgeHandling, LosslessTransform, TransformConfig};
...
/// Map zenlayout Orientation to JPEG encoder LosslessTransform.
...
pub(crate) fn execute_lossless(
...
) -> crate::jpeg_encode::error::Result<Vec<u8>> {
    ...
    crate::jpeg_encode::lossless::transform(jpeg_data, &config, stop)
}
...
pub(crate) fn execute_restructure(
    ...
    use crate::jpeg_encode::lossless::{OutputMode, RestartInterval, RestructureConfig};
    ...
    crate::jpeg_encode::lossless::restructure(jpeg_data, &config, stop)
}
...
pub(crate) fn safe_auto_orient(info: &crate::jpeg_encode::decode::JpegInfo) -> Option<u8> {
...
fn mcu_dimensions(subsampling: crate::jpeg_encode::types::Subsampling) -> (u32, u32) {
    ...
    while i + 1 < jpeg_data.len() {
        if jpeg_data[i] == 0xFF && jpeg_data[i + 1] == 0xE1 {
                ...
                && jpeg_data[data_start..data_start + 6] == *b"Exif\0\0"
            {
                ...
                crate::jpeg_encode::lossless::set_exif_orientation(&mut jpeg_data[data_start..seg_end], 1);
                return true;
            }
        }
        i += 1;
...
pub(crate) fn is_decode_ready(jpeg_data: &[u8], info: &crate::jpeg_encode::decode::JpegInfo) -> bool {
    let w = info.dimensions.width;
    let h = info.dimensions.height;
