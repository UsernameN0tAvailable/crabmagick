...
use crate::jpeg_encode::huffman::encode::{HuffmanEncodeTable, build_code_lengths, lengths_to_bits_values};
use enough::Stop;
...
pub(super) fn encode_from_coefficients(
    ...
    preserved_segments: Option<&[crate::jpeg_encode::decode::PreservedSegment]>,
...
pub(super) fn component_to_blocks(
    comp: &crate::jpeg_encode::decode::ComponentCoefficients,
...
fn category(val: i16) -> u8 {
    crate::jpeg_encode::entropy::category(val)
}
...
pub(super) fn write_sof(
    ...
    width: u32,
    height: u32,
    components: &[crate::jpeg_encode::decode::ComponentCoefficients],
) {
...
pub(super) fn write_huffman_table(
    ...
    let (bits, values) = crate::jpeg_encode::huffman::encode::lengths_to_bits_values(&table.lengths);
...
fn write_sos(output: &mut Vec<u8>, components: &[crate::jpeg_encode::decode::ComponentCoefficients]) {
    let num_components = components.len();
...
///
/// ```rust,ignore
/// use JPEG encoder::lossless::apply_exif_orientation;
///
...
pub fn apply_exif_orientation(jpeg_data: &[u8], stop: impl Stop) -> Result<Vec<u8>> {
    ...
    let mut segments: Vec<crate::jpeg_encode::decode::PreservedSegment> =
        extras.map(|e| e.segments().to_vec()).unwrap_or_default();

    // Find and rewrite EXIF orientation to 1 (Normal)
    for seg in &mut segments {
        if seg.segment_type == crate::jpeg_encode::decode::SegmentType::Exif {
