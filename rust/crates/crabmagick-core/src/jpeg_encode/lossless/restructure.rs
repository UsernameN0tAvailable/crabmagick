...
use crate::jpeg_encode::decode::{ComponentCoefficients, DecodeConfig, PreserveConfig};
use crate::jpeg_encode::encode::config::ProgressiveScan;
...
use crate::jpeg_encode::huffman::optimize::{ContextConfig, ProgressiveTokenBuffer};
use enough::Stop;
...
///
/// ```rust,ignore
/// use JPEG encoder::lossless::{restructure, RestructureConfig, OutputMode, RestartInterval};
///
...
fn write_progressive_dc_tables(
    output: &mut Vec<u8>,
    tables: &[crate::jpeg_encode::huffman::optimize::OptimizedTable],
    num_dc_tables: usize,
) {
    if num_dc_tables == 0 {
        return;
...
fn write_single_ac_dht(
    output: &mut Vec<u8>,
    table: &crate::jpeg_encode::huffman::optimize::OptimizedTable,
    slot_id: usize,
) {
    output.push(0xFF);
    output.push(MARKER_DHT);
