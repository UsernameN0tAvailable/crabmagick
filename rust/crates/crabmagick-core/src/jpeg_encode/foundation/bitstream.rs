...
use crate::jpeg_encode::error::{Error, Result, ScanRead, ScanResult};
use crate::jpeg_encode::foundation::instrumented_vec::{ProfiledVec, ProfiledVecExt};
...
impl BitWriter {
    ...
    pub fn flush_without_eoi(&mut self, output: &mut Vec<u8>) -> crate::jpeg_encode::error::Result<()> {
        self.flush();
        output.try_reserve(self.buffer.len()).map_err(|_| {
            crate::jpeg_encode::error::Error::allocation_failed(self.buffer.len(), "flush_without_eoi")
        })?;
    ...
    pub fn flush_complete_bytes_only(
        &mut self,
        output: &mut Vec<u8>,
    ) -> crate::jpeg_encode::error::Result<(u64, u8)> {
        ...
        output.try_reserve(self.buffer.len()).map_err(|_| {
            crate::jpeg_encode::error::Error::allocation_failed(self.buffer.len(), "flush_complete_bytes_only")
        })?;
        output.extend_from_slice(&self.buffer);
        self.buffer.clear();
    ...
    pub fn flush_restart_marker(&mut self, restart_num: u8) -> crate::jpeg_encode::error::Result<()> {
        // Flush pending bits
        self.flush();
...
impl<'a> BitReader<'a> {
    ...
    /// Get bits with rotate trick (like JPEG decoder's get_bits).
