...
//!
//! ```rust,ignore
//! use JPEG encoder::{StreamingEncoder, Quality, Subsampling};
//!
...
use crate::jpeg_encode::encode::config::ComputedConfig;
use crate::jpeg_encode::encode::encoder_types::HuffmanStrategy;
use crate::jpeg_encode::encode::strip::StripProcessor;
use crate::jpeg_encode::error::{Error, Result};
use crate::jpeg_encode::quant::{self, QuantTable, ZeroBiasParams};
use crate::jpeg_encode::types::{ColorSpace, JpegMode, Subsampling};
...
struct StreamingOutputState {
    ...
    writer: crate::jpeg_encode::foundation::bitstream::BitWriter,
    /// Huffman tables used for encoding (boxed: ~5.7 KB, uncommon path).
    tables: Box<crate::jpeg_encode::huffman::optimize::HuffmanTableSet>,
    /// Entropy encoding state (DC prediction, restart markers).
    entropy_state: crate::jpeg_encode::entropy::StreamingEntropyState,
...
impl StreamingEncoder {
    ...
    pub(crate) fn from_builder(builder: StreamingEncoderBuilder) -> Result<Self> {
        ...
        let quant_ctx = crate::jpeg_encode::encode::strip::QuantContext::new(
            y_quant.clone(),
            cb_quant.clone(),
            cr_quant.clone(),
            y_zero_bias,
        ...
        let config = ComputedConfig {
            ...
            edge_padding: crate::jpeg_encode::types::EdgePaddingConfig::default(),
            original_width: None,
        ...
        let streaming = if enable_streaming {
            ...
            let tables = match builder.huffman {
                ...
                HuffmanStrategy::Fixed => Box::new(crate::jpeg_encode::huffman::builtin_tables::select_tables(
                    &builder.quality,
                    builder.use_xyb,
                    builder.subsampling,
                )),
                HuffmanStrategy::FixedAnnexK => {
                    ...
                    Box::new(
                        crate::jpeg_encode::huffman::optimize::HuffmanTableSet::annex_k()
            ...
            Some(StreamingOutputState {
                writer: crate::jpeg_encode::foundation::bitstream::BitWriter::new(),
                tables,
                entropy_state: crate::jpeg_encode::entropy::StreamingEntropyState::new(),
    ...
    #[must_use]
    pub(crate) fn encode_stats(&self) -> &crate::jpeg_encode::foundation::alloc::EncodeStats {
    ...
    fn encode_new_blocks_streaming(&mut self) -> Result<()> {
        ...
        crate::jpeg_encode::entropy::encode_blocks_mcu_order(
    ...
    fn finish_streaming_static(
        ...
        output.push(0xFF);
        output.push(crate::jpeg_encode::foundation::consts::MARKER_EOI);
    ...
    fn build_jpeg_from_blocks(
        ...
        width: usize,
        height: usize,
        strip_output: crate::jpeg_encode::encode::strip::StripProcessorOutput,
        stop: impl Stop,
    ) -> Result<Vec<u8>> {
        let mut output = Vec::new();
        Self::build_jpeg_from_blocks_into(
    ...
    fn encode_sequential_xyb(
        ...
        config.write_icc_profile(output, &crate::jpeg_encode::foundation::consts::XYB_ICC_PROFILE)?;
        ...
        if matches!(config.huffman, HuffmanStrategy::Optimize) {
            ...
            let tables = crate::jpeg_encode::huffman::builtin_tables::select_tables(
                &config.quality,
                true,
                config.subsampling,
            );
...
mod tests {
    use super::*;
    use crate::jpeg_encode::encode::encoder_types::Quality;
    ...
    fn test_custom_tables_enables_streaming() {
        let tables = crate::jpeg_encode::huffman::optimize::HuffmanTableSet::from_standard().unwrap();
    ...
    fn test_streaming_round_trip_decode() {
        ...
        #[allow(deprecated)]
        let decoded = crate::jpeg_encode::decode::Decoder::new()
