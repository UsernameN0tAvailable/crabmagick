...
use crate::jpeg_encode::encode::tuning::EncodingTables;
use crate::jpeg_encode::error::Result;
use crate::jpeg_encode::types::{JpegMode, PixelFormat, Subsampling};
...
pub(crate) struct StreamingEncoderBuilder {
    ...
    /// Custom AQ map
    pub(crate) custom_aq_map: Option<crate::jpeg_encode::quant::aq::AQStrengthMap>,
...
impl StreamingEncoderBuilder {
    ...
    ///
    /// Custom tables can come from [`crate::jpeg_encode::huffman::trained`] (pre-trained on image
    /// corpora) or from a previous encoding pass via [`crate::jpeg_encode::huffman::optimize::FrequencyCounter`].
    #[must_use]
    ...
    #[must_use]
    pub(crate) fn aq_map(mut self, map: crate::jpeg_encode::quant::aq::AQStrengthMap) -> Self {
    ...
    pub(crate) fn encode(self, data: &[u8]) -> Result<Vec<u8>> {
        ...
        if data.len() != expected_size {
            return Err(crate::jpeg_encode::error::Error::invalid_buffer_size(
                expected_size,
                data.len(),
            ));
        }
