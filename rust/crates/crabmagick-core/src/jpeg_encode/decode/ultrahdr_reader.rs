...
//! use JPEG encoder::ultrahdr::{UltraHdrReaderConfig, UltraHdrMode, GainMapMemory};
//!
...
use crate::jpeg_encode::decode::{DecodedExtras, ScanlineReader};
    ...
    fn init_hdr_state(&mut self) -> Result<()> {
        ...
        match self.config.memory_strategy {
            ...
            GainMapMemory::Streaming => {
                ...
                let gm_info = crate::jpeg_encode::decode::Decoder::new()
                ...
                let gm_reader = crate::jpeg_encode::decode::Decoder::new()
...
fn decode_gainmap_jpeg(jpeg_data: &[u8]) -> Result<GainMap> {
    let decoded = crate::jpeg_encode::decode::Decoder::new().decode(jpeg_data, enough::Unstoppable)?;
