...
use crate::zenjpeg::encode::config::ComputedConfig;
use crate::zenjpeg::encode::encoder_types::HuffmanStrategy;
use crate::zenjpeg::encode::strip::StripProcessor;
use crate::zenjpeg::error::{Error, Result};
use crate::zenjpeg::quant::{self, QuantTable, ZeroBiasParams};
use crate::zenjpeg::types::{ColorSpace, JpegMode, Subsampling};
...
struct StreamingOutputState {
    ...
    writer: crate::zenjpeg::foundation::bitstream::BitWriter,
    /// Huffman tables used for encoding (boxed: ~5.7 KB, uncommon path).
    tables: Box<crate::zenjpeg::huffman::optimize::HuffmanTableSet>,
    /// Entropy encoding state (DC prediction, restart markers).
    entropy_state: crate::zenjpeg::entropy::StreamingEntropyState,
...
impl StreamingEncoder {
    ...
    pub(crate) fn from_builder(builder: StreamingEncoderBuilder) -> Result<Self> {
        ...
        let quant_ctx = crate::zenjpeg::encode::strip::QuantContext::new(
            y_quant.clone(),
            cb_quant.clone(),
            cr_quant.clone(),
            y_zero_bias,
        ...
        let config = ComputedConfig {
            ...
            edge_padding: crate::zenjpeg::types::EdgePaddingConfig::default(),
            original_width: None,
        ...
        let streaming = if enable_streaming {
            ...
            let tables = match builder.huffman {
                ...
                HuffmanStrategy::Fixed => Box::new(crate::zenjpeg::huffman::builtin_tables::select_tables(
                    &builder.quality,
                    builder.use_xyb,
                    builder.subsampling,
                )),
                HuffmanStrategy::FixedAnnexK => {
                    ...
                    Box::new(
                        crate::zenjpeg::huffman::optimize::HuffmanTableSet::annex_k()
            ...
            Some(StreamingOutputState {
                writer: crate::zenjpeg::foundation::bitstream::BitWriter::new(),
                tables,
                entropy_state: crate::zenjpeg::entropy::StreamingEntropyState::new(),
    ...
    #[must_use]
    pub(crate) fn encode_stats(&self) -> &crate::zenjpeg::foundation::alloc::EncodeStats {
    ...
    fn encode_new_blocks_streaming(&mut self) -> Result<()> {
        ...
        crate::zenjpeg::entropy::encode_blocks_mcu_order(
    ...
    fn finish_streaming_static(
        ...
        output.push(0xFF);
        output.push(crate::zenjpeg::foundation::consts::MARKER_EOI);
    ...
    fn build_jpeg_from_blocks(
        ...
        width: usize,
        height: usize,
        strip_output: crate::zenjpeg::encode::strip::StripProcessorOutput,
        stop: impl Stop,
    ) -> Result<Vec<u8>> {
        let mut output = Vec::new();
        Self::build_jpeg_from_blocks_into(
    ...
    fn encode_sequential_xyb(
        ...
        config.write_icc_profile(output, &crate::zenjpeg::foundation::consts::XYB_ICC_PROFILE)?;
        ...
        if matches!(config.huffman, HuffmanStrategy::Optimize) {
            ...
            let tables = crate::zenjpeg::huffman::builtin_tables::select_tables(
                &config.quality,
                true,
                config.subsampling,
            );
...
mod tests {
    use super::*;
    use crate::zenjpeg::encode::encoder_types::Quality;
    ...
    fn test_custom_tables_enables_streaming() {
        let tables = crate::zenjpeg::huffman::optimize::HuffmanTableSet::from_standard().unwrap();
    ...
    fn test_streaming_round_trip_decode() {
        ...
        #[allow(deprecated)]
        let decoded = crate::zenjpeg::decode::Decoder::new()
