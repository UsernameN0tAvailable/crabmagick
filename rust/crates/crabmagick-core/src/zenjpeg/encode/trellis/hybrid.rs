...
use crate::zenjpeg::encode::dct::forward_dct_8x8;
use crate::zenjpeg::encode::natural_to_zigzag_into;
use crate::zenjpeg::error::Result;
use crate::zenjpeg::foundation::consts::DCT_BLOCK_SIZE;
use crate::zenjpeg::quant::aq::AQStrengthMap;
...
pub(crate) fn get_aq_map_or_compute(
    ...
    if let Some(ref custom) = config.custom_aq_map {
        ...
        Ok(crate::zenjpeg::quant::aq::compute_aq_strength_map(
...
pub(crate) fn quantize_all_blocks_xyb_with_aq(
    ...
    let mut x_blocks = crate::zenjpeg::foundation::alloc::try_alloc_dct_blocks(num_xy_blocks, "x_blocks")?;
    let mut y_blocks = crate::zenjpeg::foundation::alloc::try_alloc_dct_blocks(num_xy_blocks, "y_blocks")?;
    let mut b_blocks = crate::zenjpeg::foundation::alloc::try_alloc_dct_blocks(num_b_blocks, "b_blocks")?;
        ...
        for mcu_x in 0..mcu_cols {
            ...
            for block_y in 0..2 {
                for block_x in 0..2 {
                    ...
                    let x_block =
                        crate::zenjpeg::encode_simd::extract_block_xyb_simd(x_plane, width, height, bx, by);
            ...
            for block_y in 0..2 {
                for block_x in 0..2 {
                    ...
                    let y_block =
                        crate::zenjpeg::encode_simd::extract_block_xyb_simd(y_plane, width, height, bx, by);
            ...
            let b_block = crate::zenjpeg::encode_simd::extract_block_xyb_simd(
