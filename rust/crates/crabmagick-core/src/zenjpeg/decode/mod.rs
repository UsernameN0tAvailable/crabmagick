...
#[allow(unused_imports)]
pub use crate::zenjpeg::types::{ColorSpace, Dimensions, JpegMode, PixelFormat};
use crate::zenjpeg::types::{Component, Subsampling};
...
use crate::zenjpeg::foundation::consts::MAX_COMPONENTS;
...
#[cfg(feature = "moxcms")]
use crate::zenjpeg::color::icc::apply_icc_transform_f32;
#[cfg(feature = "moxcms")]
use crate::zenjpeg::color::icc::{TargetColorSpace, apply_icc_transform};
...
impl DecodeConfig {
    ...
    #[must_use]
    pub fn limits(mut self, limits: crate::zenjpeg::types::Limits) -> Self {
    ...
    #[must_use]
    pub fn transform(mut self, transform: crate::zenjpeg::lossless::LosslessTransform) -> Self {
    ...
    pub fn scanline_reader<'a>(&self, data: &'a [u8]) -> Result<ScanlineReader<'a>> {
        ...
        if effective_transform != crate::zenjpeg::lossless::LosslessTransform::None {
    ...
    fn scanline_reader_owned<'a>(&self, vec: alloc::vec::Vec<u8>) -> Result<ScanlineReader<'a>> {
        use crate::zenjpeg::types::JpegMode;
        use alloc::borrow::Cow;
    ...
    fn scanline_reader_with_transform_owned<'a>(
        ...
        transform: crate::zenjpeg::lossless::LosslessTransform,
    ) -> Result<ScanlineReader<'a>> {
        use crate::zenjpeg::lossless::LosslessTransform;
    ...
    pub(crate) fn compute_effective_transform_from_data(
        &self,
        data: &[u8],
    ) -> crate::zenjpeg::lossless::LosslessTransform {
        use crate::zenjpeg::lossless::LosslessTransform;
    ...
    pub fn decode(&self, data: &[u8], stop: impl Stop) -> Result<DecodeResult> {
        ...
        // pass on the u8 output. Only Knusperli needs raw coefficients.
        {
            ...
            let needs_coefficients = self.output_target.is_f32()
                ...
                || effective_transform != crate::zenjpeg::lossless::LosslessTransform::None
        ...
        let mut result = if self.output_target.is_f32() {
            ...
            if self.output_target.is_linear() {
                crate::zenjpeg::color::icc::srgb_to_linear_inplace(&mut pixels);
            }
            ...
            if matches!(
                ...
                let dc_quant = parser.quant_tables[parser.components[0].quant_table_idx as usize]
                    .map(|qt| qt[0])
                    .unwrap_or(1);
                let strength = crate::zenjpeg::deblock::BoundaryStrength::from_dc_quant(dc_quant);
                crate::zenjpeg::deblock::filter_interleaved_u8_boundary_4tap(
                    &mut pixels,
                    width,
                    height,
                    channels,
    ...
    while pos + 4 < data.len() {
        ...
        if marker == 0xE1 && seg_end - seg_start >= EXIF_PREFIX.len() {
            ...
            if seg_data.starts_with(EXIF_PREFIX)
                && let Some(orientation) = crate::zenjpeg::lossless::parse_exif_orientation(seg_data)
            {
...
mod tests {
    use super::*;
    use crate::zenjpeg::encode::{ChromaSubsampling, EncoderConfig, PixelLayout};
    use enough::Unstoppable;
...
mod limits_tests {
    ...
    fn test_limits_from_struct() {
        let limits = crate::zenjpeg::types::Limits {
    ...
    fn test_max_scans_enforced() {
        ...
        use crate::zenjpeg::error::ErrorKind;
        use crate::zenjpeg::foundation::alloc::MAX_SCANS;
        use enough::Unstoppable;
...
mod quality_estimation_tests {
    ...
    fn test_extras_encoder_detected_as_jpegli() {
        ...
        assert!(
            matches!(
                encoder,
                crate::zenjpeg::detect::EncoderFamily::CjpegliYcbcr
                    | crate::zenjpeg::detect::EncoderFamily::CjpegliXyb
            ),
    ...
    fn test_extras_quality_estimate_confidence() {
        ...
        let probe = crate::zenjpeg::detect::probe(&jpeg).expect("probe should work");
        assert_eq!(probe.quality.confidence, crate::zenjpeg::detect::Confidence::Exact,);
    ...
    fn test_extras_ijg_quality_roundtrip() {
        ...
        for q in [10, 25, 50, 75, 85, 90, 95, 100] {
            ...
            let probe = crate::zenjpeg::detect::probe(&jpeg).expect("probe");
            assert_eq!(
                probe.quality.value, q as f32,
                "IJG Q{q} should roundtrip exactly"
            );
            assert_eq!(probe.quality.confidence, crate::zenjpeg::detect::Confidence::Exact,);
            assert_eq!(probe.quality.scale, crate::zenjpeg::detect::QualityScale::IjgQuality,);
        }
    }
    ...
    fn build_ijg_synthetic_jpeg(quality: u8) -> Vec<u8> {
        use crate::zenjpeg::detect::generate_ijg_table;
        use crate::zenjpeg::foundation::consts::{MARKER_DQT, MARKER_SOF0, MARKER_SOI, MARKER_SOS};
        ...
        for z in 0..64 {
            let ni = crate::zenjpeg::foundation::consts::JPEG_NATURAL_ORDER[z] as usize;
    ...
    fn test_extras_photoshop_detection() {
        ...
        assert_eq!(
            probe.encoder,
            crate::zenjpeg::detect::EncoderFamily::Photoshop,
    ...
    fn test_extras_probe_matches_standalone_probe() {
        ...
        let standalone_probe = crate::zenjpeg::detect::probe(&jpeg).expect("standalone probe");
