...
use crate::zenjpeg::decode::extras::AdobeColorTransform;
use crate::zenjpeg::error::{Error, Result};
use crate::zenjpeg::foundation::alloc::{checked_size_2d, try_alloc_maybeuninit};
use crate::zenjpeg::foundation::consts::{DCT_BLOCK_SIZE, DCT_SIZE, JPEG_NATURAL_ORDER};
...
impl<'a> JpegParser<'a> {
    ...
    fn to_pixels_fast_i16_subsampled(
        ...
        use crate::zenjpeg::decode::upsample::{
    ...
    fn apply_deblock_to_planes(
        ...
        for (comp_idx, info) in comp_infos.iter().enumerate() {
            ...
            if use_knusperli && comp_idx < self.coeffs.len() {
                ...
                let mut plane = crate::zenjpeg::deblock::knusperli::process_component(
                    flat_coeffs,
                    info.comp_blocks_h,
                    info.comp_blocks_v,
                    quant,
                ...
                crate::zenjpeg::deblock::filter_plane_boundary_4tap(plane, w, h, strength);
    ...
    pub(in crate::zenjpeg::decode) fn to_pixels(
        &mut self,
        format: PixelFormat,
        is_xyb: bool,
        chroma_upsampling: super::super::ChromaUpsampling,
        ...
        match (self.num_components, format) {
            ...
            (3, f) if is_rgb_family_u8(f) => {
                ...
                if is_xyb {
                    ...
                    crate::zenjpeg::color::xyb::xyb_planes_to_rgb_u8_simd(
                        &planes_f32[0],
                        &planes_f32[1],
                        &planes_f32[2],
                        &mut rgb,
    ...
    ///
    /// The `stop` parameter allows cancellation of long-running operations.
    pub(in crate::zenjpeg::decode) fn to_pixels_f32(
        &self,
        format: PixelFormat,
        is_xyb: bool,
        chroma_upsampling: super::super::ChromaUpsampling,
    ...
    pub(in crate::zenjpeg::decode) fn to_pixels_f32_deblock(
        &self,
        format: PixelFormat,
        is_xyb: bool,
        chroma_upsampling: super::super::ChromaUpsampling,
        ...
        match (self.num_components, format) {
            ...
            (3, PixelFormat::Rgb) => {
                ...
                if is_xyb {
                    ...
                    crate::zenjpeg::color::xyb::xyb_planes_to_rgb_f32_simd(
                        &planes_f32[0],
                        &planes_f32[1],
                        &planes_f32[2],
                        &mut rgb,
    ...
    pub(in crate::zenjpeg::decode) fn to_ycbcr_planes_f32(
        &self,
        chroma_upsampling: super::super::ChromaUpsampling,
