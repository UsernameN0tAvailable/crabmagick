...
use crate::zenjpeg::decode::{DecodedExtras, ScanlineReader};
    ...
    fn init_hdr_state(&mut self) -> Result<()> {
        ...
        match self.config.memory_strategy {
            ...
            GainMapMemory::Streaming => {
                ...
                let gm_info = crate::zenjpeg::decode::Decoder::new()
                ...
                let gm_reader = crate::zenjpeg::decode::Decoder::new()
...
fn decode_gainmap_jpeg(jpeg_data: &[u8]) -> Result<GainMap> {
    let decoded = crate::zenjpeg::decode::Decoder::new().decode(jpeg_data, enough::Unstoppable)?;
