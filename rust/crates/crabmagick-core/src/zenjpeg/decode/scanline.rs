...
use crate::zenjpeg::color::{ycbcr_planes_i16_to_rgb_u8, ycbcr_to_rgb, ycbcr_to_rgb_f32};
use crate::zenjpeg::deblock::BoundaryStrength;
use crate::zenjpeg::entropy::{EntropyDecoder, EntropyDecoderState};
...
use crate::zenjpeg::types::{ColorSpace, Dimensions, Subsampling};
            ...
            for mcu_x in 0..mcu_cols {
                for h in 0..h_blocks {
                    ...
                    if coeff_count <= 1 {
                        ...
                        crate::zenjpeg::quant::dequantize_unzigzag_i32_into_partial(
                            &temp_coeffs,
                            quant,
                            &mut dequant_buf,
                            coeff_count,
...
mod tests {
    ...
    fn encode_rgb(width: u32, height: u32, pixels: &[u8], quality: f32) -> Vec<u8> {
        use crate::zenjpeg::encode::v2::{ChromaSubsampling, EncoderConfig, PixelLayout};
    ...
    fn encode_rgb_subsampled(
        ...
        pixels: &[u8],
        quality: f32,
        subsampling: crate::zenjpeg::encode::v2::ChromaSubsampling,
    ) -> Vec<u8> {
        use crate::zenjpeg::encode::v2::{EncoderConfig, PixelLayout};
        use enough::Unstoppable;
    ...
    fn test_scanline_reader_rgb8() {
        use crate::zenjpeg::decode::Decoder;
    ...
    fn test_scanline_reader_420() {
        use crate::zenjpeg::decode::Decoder;
        use crate::zenjpeg::encode::v2::ChromaSubsampling;
    ...
    fn test_scanline_reader_progressive() {
        ...
        // Decode via scanline reader
        let decoder = crate::zenjpeg::decode::Decoder::new();
        let mut reader = decoder
    ...
    fn test_crop_pixel_scanline_444() {
        use crate::zenjpeg::decode::DecodeConfig;
        use crate::zenjpeg::decode::config::CropRegion;
    ...
    fn test_crop_pixel_scanline_420() {
        ...
        let jpeg = encode_rgb_subsampled(
            ...
            &pixels,
            95.0,
            crate::zenjpeg::encode::v2::ChromaSubsampling::Quarter,
        );
