...
use crate::zenjpeg::decode::{DecodeConfig, JpegInfo};
use crate::zenjpeg::encode::encoder_types::PixelLayout as EncPixelLayout;
...
fn decode_resize_encode(
    ...
    while !reader.is_finished() {
        ...
        let available = resizer
            ...
            .map_err(|e| {
                crate::zenjpeg::error::Error::new(crate::zenjpeg::error::ErrorKind::InternalError {
...
fn attach_metadata<'a>(
    mut request: crate::zenjpeg::encode::request::EncodeRequest<'a>,
    info: &'a JpegInfo,
    reset_orientation: bool,
) -> crate::zenjpeg::encode::request::EncodeRequest<'a> {
        ...
        && exif.starts_with(b"Exif\0\0")
    {
        if reset_orientation {
            ...
            crate::zenjpeg::lossless::set_exif_orientation(&mut exif_copy, 1);
...
fn drain_resizer(
    ...
    available: u32,
    encoder: &mut crate::zenjpeg::encode::byte_encoders::BytesEncoder,
    stop: &dyn Stop,
) -> Result<()> {
