...
use crate::zenjpeg::encode::encoder_types::ChromaSubsampling;
use crate::zenjpeg::error::Result;
pub use crate::zenjpeg::lossless::EdgeHandling;
...
impl<'a> LayoutRequest<'a> {
    ...
    pub fn execute(self, stop: &dyn Stop) -> Result<LayoutResult> {
        ...
        if let Some(transform) = lossless::detect_lossless(&commands) {
            let primary = if self.optimize_for_decode {
                ...
                if transform == crate::zenjpeg::lossless::LosslessTransform::None
                    && info.mode == crate::zenjpeg::types::JpegMode::Baseline
            ...
            let mut data = if transform != crate::zenjpeg::lossless::LosslessTransform::None {
            ...
            if auto_oriented && transform != crate::zenjpeg::lossless::LosslessTransform::None {
    ...
    fn detect_and_extract_gainmap(&self, info: &crate::zenjpeg::decode::JpegInfo) -> Option<Vec<u8>> {
    ...
    fn resolve_auto_orient(&self, info: &crate::zenjpeg::decode::JpegInfo) -> Vec<Command> {
        self.commands
            .iter()
            .map(|cmd| {
    ...
    fn compute_layout(
        ...
        zenlayout::compute_layout(commands, src_w, src_h, None)
            .map_err(|e| crate::zenjpeg::error::Error::invalid_config(alloc::format!("layout error: {e}")))
    }
}
...
mod tests {
    use super::*;
    use crate::zenjpeg::encode::encoder_config::EncoderConfig;
    use crate::zenjpeg::encode::encoder_types::{ChromaSubsampling, PixelLayout};
