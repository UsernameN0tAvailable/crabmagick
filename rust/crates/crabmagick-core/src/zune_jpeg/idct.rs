...
use crate::zune_jpeg::decoder::IDCTPtr;
use crate::zune_jpeg::idct::scalar::{idct_int, idct_int_1x1};
...
pub fn choose_idct_func(options: &DecoderOptions) -> IDCTPtr {
    ...
    #[cfg(target_arch = "x86_64")]
    {
        if options.use_avx2() {
            ...
            {
                debug!("Using AVX-512 integer IDCT");
                return |a: &mut [i32; 64], b: &mut [i16], c: usize| {
                    // SAFETY: avx512f + avx512bw were detected above.
                    unsafe { avx512::idct_avx512(a, b, c) }
                };
            }
    ...
    #[cfg(target_arch = "x86_64")]
    {
        if _options.use_avx2() {
            ...
            if std::is_x86_feature_detected!("avx512f") && std::is_x86_feature_detected!("avx512bw")
            {
                ...
                return |a: &mut [i32; 64], b: &mut [i16], c: usize| {
                    // SAFETY: avx512f + avx512bw were detected above.
                    unsafe { avx512::idct_avx512_4x4(a, b, c) }
                };
            }
            debug!("Using vector integer IDCT");
            return |a: &mut [i32; 64], b: &mut [i16], c: usize| {
