...
pub fn compute_mask1x1_avx2(
    ...
    for y in 0..height {
        ...
        while x < simd_end {
            // Load neighbors
            let top = crate::jxl_encoder_simd::load_f32x8(token, xyb_y, r_top + x);
            let bot = crate::jxl_encoder_simd::load_f32x8(token, xyb_y, r_bot + x);
            let left = crate::jxl_encoder_simd::load_f32x8(token, xyb_y, r_cur + x - 1);
            let right = crate::jxl_encoder_simd::load_f32x8(token, xyb_y, r_cur + x + 1);
            let center = crate::jxl_encoder_simd::load_f32x8(token, xyb_y, r_cur + x);
            ...
            crate::jxl_encoder_simd::store_f32x8(output, r_cur + x, result);
