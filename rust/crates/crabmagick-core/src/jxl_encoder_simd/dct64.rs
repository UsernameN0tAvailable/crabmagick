...
fn dct1d_4_scalar(mem: &mut [f32]) {
    let mut tmp = crate::jxl_encoder_simd::scratch_buf::<4>();
    tmp[0] = mem[0] + mem[3];
    tmp[1] = mem[1] + mem[2];
    tmp[2] = mem[0] - mem[3];
    tmp[3] = mem[1] - mem[2];
...
pub(crate) fn dct1d_64_batch(token: archmage::X64V3Token, v: &mut [magetypes::simd::f32x8; 64]) {
    ...
    crate::jxl_encoder_simd::dct32::dct1d_32_batch(token, &mut a);
    ...
    crate::jxl_encoder_simd::dct32::dct1d_32_batch(token, &mut s);
...
pub fn dct_64x32_avx2(token: archmage::X64V3Token, input: &[f32; 2048], output: &mut [f32; 2048]) {
    ...
    for batch in 0..8 {
        ...
        crate::jxl_encoder_simd::dct32::dct1d_32_batch(token, &mut v);
        for j in 0..32 {
            v[j] *= inv32;
        }
        for j in 0..32 {
