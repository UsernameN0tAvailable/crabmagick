...
fn idct1d_8_core_scalar(mem: &mut [f32]) {
    let mut tmp = crate::jxl_encoder_simd::scratch_buf::<8>();
    for i in 0..4 {
        tmp[i] = mem[2 * i];
        tmp[4 + i] = mem[2 * i + 1];
    }
...
fn idct1d_16_core_scalar(mem: &mut [f32]) {
    let mut tmp = crate::jxl_encoder_simd::scratch_buf::<16>();
    for i in 0..8 {
        tmp[i] = mem[2 * i];
        tmp[8 + i] = mem[2 * i + 1];
    }
...
fn idct1d_32_core_scalar(mem: &mut [f32]) {
    let mut tmp = crate::jxl_encoder_simd::scratch_buf::<32>();
    for i in 0..16 {
        tmp[i] = mem[2 * i];
        tmp[16 + i] = mem[2 * i + 1];
    }
...
fn idct1d_64_core_scalar(mem: &mut [f32]) {
    let mut tmp = crate::jxl_encoder_simd::scratch_buf::<64>();
    for i in 0..32 {
        tmp[i] = mem[2 * i];
        tmp[32 + i] = mem[2 * i + 1];
    }
...
pub fn idct_64x64_scalar(input: &[f32; 4096], output: &mut [f32; 4096]) {
    let mut tmp = crate::jxl_encoder_simd::scratch_buf::<4096>();
    ...
    // Transpose 64×64
    let mut transposed = crate::jxl_encoder_simd::scratch_buf::<4096>();
    for r in 0..64 {
        for c in 0..64 {
...
pub fn idct_64x32_scalar(input: &[f32; 2048], output: &mut [f32; 2048]) {
    let mut tmp = crate::jxl_encoder_simd::scratch_buf::<2048>();
    ...
    let mut transposed = crate::jxl_encoder_simd::scratch_buf::<2048>();
    for r in 0..32 {
        for c in 0..64 {
...
pub fn idct_32x64_scalar(input: &[f32; 2048], output: &mut [f32; 2048]) {
    ...
    let mut transposed2 = crate::jxl_encoder_simd::scratch_buf::<2048>();
    for r in 0..64 {
        for c in 0..32 {
...
fn gather_col(
    ...
    crate::jxl_encoder_simd::gather_col_strided(token, data, base_row, j, stride)
}
...
fn scatter_col(
    ...
    crate::jxl_encoder_simd::scatter_col_strided(v, data, base_row, j, stride)
}
...
pub(crate) fn idct1d_64_core_batch(
    ...
    crate::jxl_encoder_simd::idct32::idct1d_32_core_batch(token, &mut second);
    ...
    crate::jxl_encoder_simd::idct32::idct1d_32_core_batch(token, &mut first);
...
pub fn idct_64x32_avx2(token: archmage::X64V3Token, input: &[f32; 2048], output: &mut [f32; 2048]) {
    ...
    for batch in 0..8 {
        ...
        crate::jxl_encoder_simd::idct32::idct1d_32_batch(token, &mut v);
        for j in 0..32 {
...
mod tests {
    ...
    fn test_dct64_idct64_roundtrip() {
        ...
        crate::jxl_encoder_simd::dct64::dct_64x64(&input, &mut coeffs);
    ...
    fn test_dct64x32_idct64x32_roundtrip() {
        ...
        crate::jxl_encoder_simd::dct64::dct_64x32(&input, &mut coeffs);
    ...
    fn test_dct32x64_idct32x64_roundtrip() {
        ...
        let mut coeffs = [0.0f32; 2048];
        crate::jxl_encoder_simd::dct64::dct_32x64(&input, &mut coeffs);
