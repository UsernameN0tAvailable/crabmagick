...
pub fn entropy_coeffs_avx2(
    ...
    for chunk in 0..chunks {
        ...
        let bc = crate::jxl_encoder_simd::load_f32x8(token, block_c_s, base);
        let by_v = crate::jxl_encoder_simd::load_f32x8(token, block_y_s, base);
        let w = crate::jxl_encoder_simd::load_f32x8(token, weights_s, base);
        let iw = crate::jxl_encoder_simd::load_f32x8(token, inv_weights_s, base);
        ...
        if pixel_domain {
            let err = w * diff;
            crate::jxl_encoder_simd::store_f32x8(error_coeffs_s, base, err);
        }
