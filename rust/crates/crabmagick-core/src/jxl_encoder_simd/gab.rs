...
pub fn gab_smooth_avx2(
    ...
    for y in 0..height {
        ...
        while x < simd_end {
            ...
            let center = crate::jxl_encoder_simd::load_f32x8(token, input, row_c + x);
            let top = crate::jxl_encoder_simd::load_f32x8(token, input, row_t + x);
            let bottom = crate::jxl_encoder_simd::load_f32x8(token, input, row_b + x);
            let left = crate::jxl_encoder_simd::load_f32x8(token, input, row_c + x - 1);
            let right = crate::jxl_encoder_simd::load_f32x8(token, input, row_c + x + 1);
            let tl = crate::jxl_encoder_simd::load_f32x8(token, input, row_t + x - 1);
            let tr = crate::jxl_encoder_simd::load_f32x8(token, input, row_t + x + 1);
            let bl = crate::jxl_encoder_simd::load_f32x8(token, input, row_b + x - 1);
            let br = crate::jxl_encoder_simd::load_f32x8(token, input, row_b + x + 1);
            ...
            // Store 8 results
            crate::jxl_encoder_simd::store_f32x8(output, row_c + x, result);
