// Copyright (c) Laurenz Stampfl (hayro-jpeg2000). Licensed MIT OR Apache-2.0.
#![allow(warnings, clippy::all, unexpected_cfgs)]
/*!
A memory-safe, pure-Rust JPEG 2000 decoder for IIIF workloads.

JPEG 2000 decoder can decode both raw JPEG 2000 codestreams (`.j2c`) and images wrapped
...
into a simple 8-bit image with either greyscale, RGB or CMYK color data.

# Example
```rust,no_run
use crate::jpeg2000_decode::{DecodeSettings, DecoderContext, Image};
...
at the original example in [GitHub](https://github.com/LaurenzV/hayro/blob/main/hayro-jpeg2000/examples/png.rs),
...
// Unsafe is permitted only in j2c::decode for the parallel tile scatter (disjoint
// writes to non-overlapping tile regions — same pattern as JXL encoder transform.rs).
#![deny(unsafe_code)]
#![forbid(missing_docs)]
...
use crate::jpeg2000_decode::error::{bail, err};
use crate::jpeg2000_decode::j2c::Header;
use crate::jpeg2000_decode::jp2::ImageBoxes;
use crate::jpeg2000_decode::jp2::cdef::{ChannelAssociation, ChannelType};
use crate::jpeg2000_decode::jp2::cmap::ComponentMappingType;
use crate::jpeg2000_decode::jp2::colr::{CieLab, EnumeratedColorspace};

pub mod api;
pub(crate) mod arch {
    /// True at runtime if AVX2 is available.
    #[inline(always)]
    ...
    /// True at runtime if NEON is available (always true on aarch64).
    #[inline(always)]
...
use crate::jpeg2000_decode::math::{Level, SIMD_WIDTH, Simd, dispatch, f32x8};
...
impl<'a> Image<'a> {
    ...
    #[inline]
    pub(crate) fn new(data: &'a [u8], settings: &DecodeSettings) -> Result<Self> {
...
pub struct Bitmap {
    ...
    /// JPEG 2000 decoder always scales to 8-bit).
    ///
...
impl DecodedImage<'_> {
    ...
    #[inline]
    pub(crate) fn data_u8(&self) -> Vec<u8> {
...
fn get_color_space(boxes: &ImageBoxes, num_components: usize) -> Result<ColorSpace> {
    ...
    {
        jp2::colr::ColorSpace::Enumerated(e) => match e {
            EnumeratedColorspace::Cmyk => ColorSpace::CMYK,
            EnumeratedColorspace::Srgb => ColorSpace::RGB,
            EnumeratedColorspace::RommRgb => ColorSpace::RGB,
            ...
            EnumeratedColorspace::CieLab(_) => ColorSpace::RGB,
            _ => bail!(FormatError::Unsupported),
        },
        jp2::colr::ColorSpace::Icc(icc) => match crate::jpeg2000_decode::jp2::icc::ICCMetadata::from_data(icc)
            .map(|metadata| metadata.color_space.num_components() as usize)
            .unwrap_or(3)
        {
            1 => ColorSpace::Gray,
            3 => ColorSpace::RGB,
            4 => ColorSpace::CMYK,
            n => ColorSpace::Unknown {
...
/// Decode a JP2 container image or bare J2K codestream into packed 8-bit pixels.
#[inline]
pub(crate) fn decode(data: &[u8]) -> Result<api::DecodedImage> {
    api::decode(data)
}
