...
use crate::jxl_encode_simd::entropy::EntropyCoeffResult;
...
pub fn fused_dct8_entropy_fallback(
    ...
    let mut input = crate::jxl_encode_simd::scratch_buf::<64>();
    let x0 = bx * 8;
    for dy in 0..8 {
    ...
    let mut dct = crate::jxl_encode_simd::scratch_buf::<64>();
    crate::jxl_encode_simd::dct_8x8(&input, &mut dct);
    ...
    crate::jxl_encode_simd::entropy_estimate_coeffs(
        &dct,
        y_dct,
        weights,
        inv_weights,
...
pub fn fused_dct8_entropy_avx2(
    ...
    let r0 = crate::jxl_encode_simd::load_f32x8(token, plane, base);
    let r1 = crate::jxl_encode_simd::load_f32x8(token, plane, base + plane_stride);
    let r2 = crate::jxl_encode_simd::load_f32x8(token, plane, base + 2 * plane_stride);
    let r3 = crate::jxl_encode_simd::load_f32x8(token, plane, base + 3 * plane_stride);
    let r4 = crate::jxl_encode_simd::load_f32x8(token, plane, base + 4 * plane_stride);
    let r5 = crate::jxl_encode_simd::load_f32x8(token, plane, base + 5 * plane_stride);
    let r6 = crate::jxl_encode_simd::load_f32x8(token, plane, base + 6 * plane_stride);
    let r7 = crate::jxl_encode_simd::load_f32x8(token, plane, base + 7 * plane_stride);

    // Column DCT
    let (r0, r1, r2, r3, r4, r5, r6, r7) =
        crate::jxl_encode_simd::dct8::vectorized_dct1d_8(token, r0, r1, r2, r3, r4, r5, r6, r7);
    ...
    // Transpose
    let (r0, r1, r2, r3, r4, r5, r6, r7) =
        crate::jxl_encode_simd::dct8::transpose_8x8_regs(token, r0, r1, r2, r3, r4, r5, r6, r7);
    ...
    // Store DCT output if requested (Y channel for CfL reference)
    if let Some(out) = dct_output {
        crate::jxl_encode_simd::store_f32x8(out, 0, r0);
        crate::jxl_encode_simd::store_f32x8(out, 8, r1);
        crate::jxl_encode_simd::store_f32x8(out, 16, r2);
        crate::jxl_encode_simd::store_f32x8(out, 24, r3);
        crate::jxl_encode_simd::store_f32x8(out, 32, r4);
        crate::jxl_encode_simd::store_f32x8(out, 40, r5);
        crate::jxl_encode_simd::store_f32x8(out, 48, r6);
        crate::jxl_encode_simd::store_f32x8(out, 56, r7);
    }
    ...
    macro_rules! process_row {
        ($dct_row:expr, $row_idx:expr) => {{
            let base = $row_idx * 8;
            let w = crate::jxl_encode_simd::load_f32x8(token, weights, base);
            let iw = crate::jxl_encode_simd::load_f32x8(token, inv_weights, base);
            let y = crate::jxl_encode_simd::load_f32x8(token, y_dct, base);
            ...
            let err = w * diff;
            crate::jxl_encode_simd::store_f32x8(error_coeffs, base, err);
...
mod tests {
    ...
    fn test_fused_matches_separate() {
        ...
        crate::jxl_encode_simd::dct_8x8(&input, &mut sep_dct);
        sep_dct[0] = 0.0;

        let mut sep_error = [0.0f32; 64];
        let sep_result = crate::jxl_encode_simd::entropy_estimate_coeffs(
            &sep_dct,
            &y_dct,
            &weights,
            &inv_weights,
    ...
    fn test_fused_cfl_matches_separate() {
        ...
        let mut y_dct = [0.0f32; 64];
        {
            ...
            crate::jxl_encode_simd::dct_8x8(&y_input, &mut y_dct);
            y_dct[0] = 0.0;
        }
