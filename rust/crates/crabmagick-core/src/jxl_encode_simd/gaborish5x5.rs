...
pub fn gaborish_5x5_avx2(
    ...
    for y in 0..height {
        ...
        while x < simd_end {
            // Center
            let center = crate::jxl_encode_simd::load_f32x8(token, row_0, x);

            // r: 4 orthogonal at distance 1
            let left1 = crate::jxl_encode_simd::load_f32x8(token, row_0, x - 1);
            let right1 = crate::jxl_encode_simd::load_f32x8(token, row_0, x + 1);
            let top1 = crate::jxl_encode_simd::load_f32x8(token, row_m1, x);
            let bot1 = crate::jxl_encode_simd::load_f32x8(token, row_p1, x);
            let r_sum = left1 + right1 + top1 + bot1;

            // d: 4 diagonal at distance sqrt(2)
            let tl1 = crate::jxl_encode_simd::load_f32x8(token, row_m1, x - 1);
            let tr1 = crate::jxl_encode_simd::load_f32x8(token, row_m1, x + 1);
            let bl1 = crate::jxl_encode_simd::load_f32x8(token, row_p1, x - 1);
            let br1 = crate::jxl_encode_simd::load_f32x8(token, row_p1, x + 1);
            let d_sum = tl1 + tr1 + bl1 + br1;

            // R: 4 orthogonal at distance 2
            let left2 = crate::jxl_encode_simd::load_f32x8(token, row_0, x - 2);
            let right2 = crate::jxl_encode_simd::load_f32x8(token, row_0, x + 2);
            let top2 = crate::jxl_encode_simd::load_f32x8(token, row_m2, x);
            let bot2 = crate::jxl_encode_simd::load_f32x8(token, row_p2, x);
            let big_r_sum = left2 + right2 + top2 + bot2;

            // L: 8 knight's move neighbors
            let l_a = crate::jxl_encode_simd::load_f32x8(token, row_m1, x - 2);
            let l_b = crate::jxl_encode_simd::load_f32x8(token, row_p1, x - 2);
            let l_c = crate::jxl_encode_simd::load_f32x8(token, row_m1, x + 2);
            let l_d = crate::jxl_encode_simd::load_f32x8(token, row_p1, x + 2);
            let l_e = crate::jxl_encode_simd::load_f32x8(token, row_m2, x - 1);
            let l_f = crate::jxl_encode_simd::load_f32x8(token, row_m2, x + 1);
            let l_g = crate::jxl_encode_simd::load_f32x8(token, row_p2, x - 1);
            let l_h = crate::jxl_encode_simd::load_f32x8(token, row_p2, x + 1);
            let l_sum = l_a + l_b + l_c + l_d + l_e + l_f + l_g + l_h;

            // D: 4 corner at distance 2*sqrt(2)
            let tl2 = crate::jxl_encode_simd::load_f32x8(token, row_m2, x - 2);
            let tr2 = crate::jxl_encode_simd::load_f32x8(token, row_m2, x + 2);
            let bl2 = crate::jxl_encode_simd::load_f32x8(token, row_p2, x - 2);
            let br2 = crate::jxl_encode_simd::load_f32x8(token, row_p2, x + 2);
            ...
            crate::jxl_encode_simd::store_f32x8(output, r_0 + x, result);
