...
// ============================================================================
// Scalar fallback — matches jxl_encode/src/vardct/dct/forward.rs exactly
// ============================================================================

#[inline(always)]
pub fn dct_16x16_scalar(input: &[f32; 256], output: &mut [f32; 256]) {
    let mut tmp = crate::jxl_encode_simd::scratch_buf::<256>();
    ...
    // Transpose 16x16
    let mut transposed = crate::jxl_encode_simd::scratch_buf::<256>();
    for r in 0..16 {
        for c in 0..16 {
...
pub fn dct_16x8_scalar(input: &[f32; 128], output: &mut [f32; 128]) {
    let mut tmp = crate::jxl_encode_simd::scratch_buf::<128>();
    ...
    // Transpose 16x8 -> 8x16
    let mut transposed = crate::jxl_encode_simd::scratch_buf::<128>();
    for row in 0..16 {
        for col in 0..8 {
...
pub fn dct_8x16_neon(token: archmage::NeonToken, input: &[f32; 128], output: &mut [f32; 128]) {
    ...
    let mut pass2_out = crate::jxl_encode_simd::scratch_buf::<128>();
    for batch in 0..4 {
