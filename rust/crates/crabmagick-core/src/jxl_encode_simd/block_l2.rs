    ...
    for by in 0..ysize_blocks {
        for bx in 0..xsize_blocks {
            ...
            for py in 0..8 {
                ...
                let mask_v = crate::jxl_encode_simd::load_f32x8(token, mask1x1, row_start);
                let mask_sq = mask_v * mask_v;

                // X channel: w_x * mask_sq * (orig_x - recon_x)^2
                let orig_x = crate::jxl_encode_simd::load_f32x8(token, original[0], row_start);
                let recon_x = crate::jxl_encode_simd::load_f32x8(token, reconstructed[0], row_start);
                let diff_x = orig_x - recon_x;
                acc += w_x * mask_sq * diff_x * diff_x;

                // Y channel: w_y * mask_sq * (orig_y - recon_y)^2
                let orig_y = crate::jxl_encode_simd::load_f32x8(token, original[1], row_start);
                let recon_y = crate::jxl_encode_simd::load_f32x8(token, reconstructed[1], row_start);
                let diff_y = orig_y - recon_y;
                // w_y = 1.0, so skip the multiply
                acc += mask_sq * diff_y * diff_y;

                // B channel: w_b * mask_sq * (orig_b - recon_b)^2
                let orig_b = crate::jxl_encode_simd::load_f32x8(token, original[2], row_start);
                let recon_b = crate::jxl_encode_simd::load_f32x8(token, reconstructed[2], row_start);
