...
//! Common helpers used in various places of JXL decoder.
//!
...
macro_rules! expand_u32 {
    ($bitstream:ident; $($rest:tt)*) => {
        $crate::jxl_decode::jxl_oxide_common::expand_u32!(@convert $bitstream; (); ($($rest)*,))
    };
    (@convert $bitstream:ident; ($($done:tt)*); ()) => {
        $bitstream.read_u32($($done)*)
    };
    (@convert $bitstream:ident; ($($done:tt)*); ($c:literal, $($rest:tt)*)) => {
        $crate::jxl_decode::jxl_oxide_common::expand_u32!(@convert $bitstream; ($($done)* $c,); ($($rest)*))
    };
    (@convert $bitstream:ident; ($($done:tt)*); (u($n:literal), $($rest:tt)*)) => {
        $crate::jxl_decode::jxl_oxide_common::expand_u32!(@convert $bitstream; ($($done)* crate::jxl_decode::jxl_bitstream::U($n),); ($($rest)*))
    };
    (@convert $bitstream:ident; ($($done:tt)*); ($c:literal + u($n:literal), $($rest:tt)*)) => {
        $crate::jxl_decode::jxl_oxide_common::expand_u32!(@convert $bitstream; ($($done)* $c + crate::jxl_decode::jxl_bitstream::U($n),); ($($rest)*))
    };
}
...
macro_rules! read_bits {
    ($bistream:ident, $c:literal $(, $ctx:expr)?) => {
        crate::jxl_decode::jxl_bitstream::BitstreamResult::Ok($c)
    };
    ($bitstream:ident, u($n:literal) $(, $ctx:expr)?) => {
        $bitstream.read_bits($n)
    };
    ($bitstream:ident, u($n:literal); UnpackSigned $(, $ctx:expr)?) => {
        $bitstream.read_bits($n).map(crate::jxl_decode::jxl_bitstream::unpack_signed)
    };
    ...
    ($bitstream:ident, $c:literal + u($n:literal); UnpackSigned $(, $ctx:expr)?) => {
        $bitstream.read_bits($n).map(|v| crate::jxl_decode::jxl_bitstream::unpack_signed(v.wrapping_add($c)))
    };
    ($bitstream:ident, U32($($args:tt)+) $(, $ctx:expr)?) => {
        $crate::jxl_decode::jxl_oxide_common::expand_u32!($bitstream; $($args)+)
    };
    ($bitstream:ident, U32($($args:tt)+); UnpackSigned $(, $ctx:expr)?) => {
        $crate::jxl_decode::jxl_oxide_common::expand_u32!($bitstream; $($args)+).map(crate::jxl_decode::jxl_bitstream::unpack_signed)
    };
    ($bitstream:ident, U64 $(, $ctx:expr)?) => {
        $bitstream.read_u64()
    };
    ($bitstream:ident, U64; UnpackSigned $(, $ctx:expr)?) => {
        $bitstream.read_u64().map(crate::jxl_decode::jxl_bitstream::unpack_signed_u64)
    };
    ...
    ($bitstream:ident, Vec[$($inner:tt)*]; $count:expr $(, $ctx:expr)?) => {
        {
            ...
            (0..count)
                .into_iter()
                .map(|_| $crate::jxl_decode::jxl_oxide_common::read_bits!($bitstream, $($inner)* $(, $ctx)?))
    ...
    ($bitstream:ident, Array[$($inner:tt)*]; $count:expr $(, $ctx:expr)?) => {
        (|| -> ::std::result::Result<[_; $count], _> {
            ...
            for point in &mut ret {
                *point = match $crate::jxl_decode::jxl_oxide_common::read_bits!($bitstream, $($inner)* $(, $ctx)?) {
...
macro_rules! make_def {
    ...
    (@ty; Vec[$($inner:tt)*]; $count:expr) => { Vec<$crate::jxl_decode::jxl_oxide_common::make_def!(@ty; $($inner)*)> };
    (@ty; Array[$($inner:tt)*]; $count:expr) => { [$crate::jxl_decode::jxl_oxide_common::make_def!(@ty; $($inner)*); $count] };
    ($(#[$attrs:meta])* $v:vis struct $bundle_name:ident {
        ...
        $v struct $bundle_name {
            $($(#[$fieldattrs])* $vfield $field: $crate::jxl_decode::jxl_oxide_common::make_def!(@ty; $($expr)*),)*
        }
    };
}
...
macro_rules! make_parse {
    (@parse $bitstream:ident; cond($cond:expr); default($def_expr:expr); ty($($spec:tt)*); ctx($ctx:expr)) => {
        if $cond {
            $crate::jxl_decode::jxl_oxide_common::read_bits!($bitstream, $($spec)*, $ctx)?
        } else {
            $def_expr
        }
    };
    (@parse $bitstream:ident; cond($cond:expr); ty($($spec:tt)*); ctx($ctx:expr)) => {
        if $cond {
            ...
            <$crate::jxl_decode::jxl_oxide_common::make_def!(@ty; $($spec)*)>::default_with_context($ctx)
        }
    };
    ...
    (@select_error_ty;) => {
        crate::jxl_decode::jxl_bitstream::Error
    };
    ...
    ($bundle_name:ident $(error($err:ty))? {
        ...
        impl<Ctx: Copy> $crate::jxl_decode::jxl_oxide_common::Bundle<Ctx> for $bundle_name {
            type Error = $crate::jxl_decode::jxl_oxide_common::make_parse!(@select_error_ty; $($err)?);

            #[allow(unused)]
            fn parse(bitstream: &mut crate::jxl_decode::jxl_bitstream::Bitstream, ctx: Ctx) -> ::std::result::Result<Self, Self::Error> where Self: Sized {
                use $crate::jxl_decode::jxl_oxide_common::{Bundle, BundleDefault};
                $(
                    let $field = $crate::jxl_decode::jxl_oxide_common::make_parse!(
                        @parse bitstream;
                        $(cond($cond);)?
                        $(default($def_expr);)?
                        ty($($expr)*);
                        ctx($crate::jxl_decode::jxl_oxide_common::make_parse!(@select_ctx; ctx; $($ctx_for_field)?))
                    );
                )*
                Ok(Self { $($field,)* })
            }
        ...
        impl<Ctx: Copy> $crate::jxl_decode::jxl_oxide_common::BundleDefault<Ctx> for $bundle_name {
            #[allow(unused)]
            fn default_with_context(_ctx: Ctx) -> Self where Self: Sized {
                use $crate::jxl_decode::jxl_oxide_common::BundleDefault;
                $(
                    let $field = $crate::jxl_decode::jxl_oxide_common::make_parse!(
                        @default($($expr)*);
                        $($def_expr)?;
                        $crate::jxl_decode::jxl_oxide_common::make_parse!(@select_ctx; _ctx; $($ctx_for_field)?)
                    );
                )*
                Self { $($field,)* }
            }
    ...
    ($bundle_name:ident ctx($ctx_id:ident : $ctx:ty) $(error($err:ty))? {
        ...
        impl $crate::jxl_decode::jxl_oxide_common::Bundle<$ctx> for $bundle_name {
            type Error = $crate::jxl_decode::jxl_oxide_common::make_parse!(@select_error_ty; $($err)?);

            #[allow(unused)]
            fn parse(bitstream: &mut crate::jxl_decode::jxl_bitstream::Bitstream, $ctx_id: $ctx) -> ::std::result::Result<Self, Self::Error> where Self: Sized {
                use $crate::jxl_decode::jxl_oxide_common::{Bundle, BundleDefault};
                $(
                    let $field = $crate::jxl_decode::jxl_oxide_common::make_parse!(
                        @parse bitstream;
                        ...
                        $(default($def_expr);)?
                        ty($($expr)*);
                        ctx($crate::jxl_decode::jxl_oxide_common::make_parse!(@select_ctx; $ctx_id; $($ctx_for_field)?))
                    );
                )*
                Ok(Self { $($field,)* })
            }
        ...
        impl $crate::jxl_decode::jxl_oxide_common::BundleDefault<$ctx> for $bundle_name {
            #[allow(unused)]
            fn default_with_context($ctx_id: $ctx) -> Self where Self: Sized {
                ...
                $(
                    let $field = $crate::jxl_decode::jxl_oxide_common::make_parse!(
                        @default($($expr)*);
                        $($def_expr)?;
                        $crate::jxl_decode::jxl_oxide_common::make_parse!(@select_ctx; $ctx_id; $($ctx_for_field)?)
                    );
                )*
                Self { $($field,)* }
            }
...
macro_rules! define_bundle {
    (
        ...
        $(
            $crate::jxl_decode::jxl_oxide_common::make_def!($(#[$attrs])* $v struct $bundle_name { $($body)* });
            $crate::jxl_decode::jxl_oxide_common::make_parse!($bundle_name $(aligned($aligned))? $(ctx($ctx_id: $ctx))? $(error($err))? { $($body)* });
        )*
    };
}
...
impl<Ctx> Bundle<Ctx> for Name {
    type Error = crate::jxl_decode::jxl_bitstream::Error;
    ...
    fn parse(bitstream: &mut Bitstream, _: Ctx) -> Result<Self, Self::Error> {
        ...
        let name = String::from_utf8(data)
            .map_err(|_| crate::jxl_decode::jxl_bitstream::Error::ValidationFailed("non-UTF-8 name"))?;
        Ok(Self(name))
    }
}
