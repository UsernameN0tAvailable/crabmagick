...
pub(super) fn gamut_map(
    ...
    let (r, g, b) = if is_x86_feature_detected!("fma") && is_x86_feature_detected!("sse4.1") {
        ...
        unsafe {
            ...
            for ((r, g), b) in (&mut r_it).zip(&mut g_it).zip(&mut b_it) {
                ...
                let [vr, vg, vb] =
                    crate::jxl_decode::jxl_color::gamut::map_gamut_x86_64_sse2(rgb, luminances, saturation_factor);
    ...
    for ((r, g), b) in r.iter_mut().zip(g).zip(b) {
        let mapped = crate::jxl_decode::jxl_color::gamut::map_gamut_generic([*r, *g, *b], luminances, saturation_factor);
        *r = mapped[0];
        *g = mapped[1];
        *b = mapped[2];
    }
...
pub(super) unsafe fn gamut_map_x86_64_avx2<'r, 'g, 'b>(
    ...
    for ((r, g), b) in (&mut r_it).zip(&mut g_it).zip(&mut b_it) {
        ...
        let [vr, vg, vb] = crate::jxl_decode::jxl_color::gamut::map_gamut_x86_64_avx2(rgb, luminances, saturation_factor);
...
pub(super) unsafe fn gamut_map_x86_64_fma<'r, 'g, 'b>(
    ...
    for ((r, g), b) in (&mut r_it).zip(&mut g_it).zip(&mut b_it) {
        ...
        let [vr, vg, vb] = crate::jxl_decode::jxl_color::gamut::map_gamut_x86_64_fma(rgb, luminances, saturation_factor);
        std::arch::x86_64::_mm_storeu_ps(r.as_mut_ptr(), vr);
        std::arch::x86_64::_mm_storeu_ps(g.as_mut_ptr(), vg);
        std::arch::x86_64::_mm_storeu_ps(b.as_mut_ptr(), vb);
    }
...
pub(super) unsafe fn gamut_map_aarch64_neon<'r, 'g, 'b>(
    ...
    for ((r, g), b) in (&mut r_it).zip(&mut g_it).zip(&mut b_it) {
        ...
        let [vr, vg, vb] = crate::jxl_decode::jxl_color::gamut::map_gamut_aarch64_neon(rgb, luminances, saturation_factor);
