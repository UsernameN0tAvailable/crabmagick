...
impl ColorEncodingWithProfile {
    ...
    pub fn with_icc(icc_profile: &[u8]) -> Result<Self> {
        match crate::jxl_decode::jxl_color::icc::parse_icc(icc_profile) {
            Ok(encoding) => Ok(Self::new(encoding)),
            Err(Error::UnsupportedIccProfile) => {
                let raw = crate::jxl_decode::jxl_color::icc::parse_icc_raw(icc_profile)?;
                Ok(Self {
...
impl ColorTransform {
    ...
    fn with_builder<Cms: ColorManagementSystem + ?Sized>(
        ...
        let mut current_encoding = match from.encoding {
            ColourEncoding::IccProfile(_) => {
                let rendering_intent = crate::jxl_decode::jxl_color::icc::parse_icc_raw(&from.icc_profile)?
                    .header
                    .rendering_intent;
                match &to.encoding {
                    ColourEncoding::IccProfile(_) => {
                        ...
                        let transform = cms
                            ...
                            .map_err(crate::jxl_decode::jxl_color::Error::CmsFailure)?;
                        return Ok(Self {
                            begin_channels,
            ...
            ColourEncoding::Enum(
                ...
                let mat = crate::jxl_decode::jxl_color::ciexyz::primaries_to_xyz_mat(primaries, white_point);
            ...
            ColourEncoding::Enum(
                ...
                let mat = crate::jxl_decode::jxl_color::ciexyz::primaries_to_xyz_mat(PRIMARIES_SRGB, white_point);
                let luminances = [mat[3], mat[4], mat[5]];
                ...
                && current_encoding.primaries != target_encoding.primaries)
        {
            if current_encoding.rendering_intent == RenderingIntent::Perceptual {
                let mat = crate::jxl_decode::jxl_color::ciexyz::primaries_to_xyz_mat(
            ...
            match current_encoding.colour_space {
                ...
                ColourSpace::Grey => {
                    // Yxy to XYZ
                    let illuminant =
                        crate::jxl_decode::jxl_color::ciexyz::AsIlluminant::as_illuminant(&current_encoding.white_point);
            ...
            match target_encoding.colour_space {
                ColourSpace::Rgb => {
                    // XYZ to RGB
                    let mat = crate::jxl_decode::jxl_color::ciexyz::xyz_to_primaries_mat(
    ...
    pub fn xyb_to_enum(
        ...
        Self::new(
            ...
            oim,
            tone_mapping,
            &crate::jxl_decode::jxl_color::NullCms, // XYB to enum colorspace shouldn't generate ICC op
        )
    }
    ...
    fn run(&self, channels: &mut [&mut [f32]], num_input_channels: usize) -> Result<usize> {
        ...
        Ok(match self {
            Self::XybToMixedLms {
                ...
                crate::jxl_decode::jxl_color::xyb::run(xyb, *opsin_bias, *intensity_target);
                3
            }
