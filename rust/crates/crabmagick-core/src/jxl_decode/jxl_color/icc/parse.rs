...
impl IccProfileInfo {
    ...
    fn chad_inv(&self) -> [f32; 9] {
        ...
        crate::jxl_decode::jxl_color::ciexyz::matinv(&chad_mat)
    }

    pub fn primaries(&self) -> Option<Primaries> {
        const PRIMARIES_TO_ENUM: [([[f32; 2]; 3], Primaries); 3] = [
            (crate::jxl_decode::jxl_color::consts::PRIMARIES_SRGB, Primaries::Srgb),
            (crate::jxl_decode::jxl_color::consts::PRIMARIES_P3, Primaries::P3),
            (crate::jxl_decode::jxl_color::consts::PRIMARIES_BT2100, Primaries::Bt2100),
        ];
        ...
        let xyz_rgb_mat_adapted = crate::jxl_decode::jxl_color::ciexyz::matmul3(&chad_inv, &xyz_rgb_mat);
    ...
    pub fn white_point(&self) -> WhitePoint {
        const WP_TO_ENUM: [([f32; 2], WhitePoint); 3] = [
            (crate::jxl_decode::jxl_color::consts::ILLUMINANT_D65, WhitePoint::D65),
            (crate::jxl_decode::jxl_color::consts::ILLUMINANT_DCI, WhitePoint::Dci),
            (crate::jxl_decode::jxl_color::consts::ILLUMINANT_E, WhitePoint::E),
        ];

        let chad_inv = self.chad_inv();
        let ill_xyz = crate::jxl_decode::jxl_color::ciexyz::matmul3vec(&chad_inv, &self.media_white());
...
fn validate_chad(chad_s15fixed16: [i32; 9]) -> Result<()> {
    ...
    let chad_inv = crate::jxl_decode::jxl_color::ciexyz::matinv(&mat_float);
    for v in chad_inv {
        if !v.is_finite() {
            return Err(Error::IccParseFailure("invalid chad tag"));
        }
