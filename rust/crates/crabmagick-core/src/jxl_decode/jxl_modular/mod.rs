...
define_bundle! {
    #[derive(Debug, Clone)]
    struct ModularHeader error(crate::jxl_decode::jxl_modular::Error) {
...
fn read_and_validate_local_modular_header(
    ...
    if header.nb_transforms > 512 {
        ...
        return Err(crate::jxl_decode::jxl_bitstream::Error::ProfileConformance("nb_transforms too large").into());
    }
    ...
    if nb_channels_tr > (1 << 16) {
        ...
        return Err(crate::jxl_decode::jxl_bitstream::Error::ProfileConformance("nb_channels_tr too large").into());
    }

    let ma_ctx = if header.use_global_tree {
        global_ma_config
            .ok_or(crate::jxl_decode::jxl_modular::Error::GlobalMaTreeNotAvailable)?
            .clone()
    } else {
