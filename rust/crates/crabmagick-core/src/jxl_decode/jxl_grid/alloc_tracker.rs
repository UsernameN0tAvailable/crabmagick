...
impl AllocTracker {
    ...
    pub fn alloc<T>(&self, count: usize) -> Result<AllocHandle, crate::jxl_decode::jxl_grid::OutOfMemory> {
        ...
        match result {
            ...
            Err(left) => {
                ...
                Err(crate::jxl_decode::jxl_grid::OutOfMemory::new(bytes))
            }
        }
    }
    ...
    /// limit.
    pub fn shrink_limit(&self, by_bytes: usize) -> Result<(), crate::jxl_decode::jxl_grid::OutOfMemory> {
        let result = self.inner.bytes_left.fetch_update(
            Ordering::Relaxed,
            Ordering::Relaxed,
        ...
        if result.is_ok() {
            ...
            Err(crate::jxl_decode::jxl_grid::OutOfMemory::new(by_bytes))
        }
    }
}
