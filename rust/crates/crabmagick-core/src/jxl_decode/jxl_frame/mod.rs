...
use crate::jxl_decode::jxl_grid::{AllocHandle, AllocTracker};
use crate::jxl_decode::jxl_image::ImageHeader;
use crate::jxl_decode::jxl_oxide_common::Bundle;
...
use crate::jxl_decode::jxl_modular::Sample;
use crate::jxl_decode::jxl_modular::{MaConfig, image::TransformedModularSubimage};
use crate::jxl_decode::jxl_threadpool::JxlThreadPool;

use crate::jxl_decode::jxl_frame::data::*;
...
impl Bundle<FrameContext<'_>> for Frame {
    ...
    fn parse(bitstream: &mut Bitstream, ctx: FrameContext) -> Result<Self> {
        ...
        if width > (1 << 30) {
            ...
            return Err(crate::jxl_decode::jxl_bitstream::Error::ProfileConformance("frame width too large").into());
        }
        if height > (1 << 30) {
            tracing::error!(width, "Frame height too large; limit is 2^30");
            return Err(crate::jxl_decode::jxl_bitstream::Error::ProfileConformance("frame height too large").into());
        }
        if (width * height) > (1 << 40) {
            ...
            return Err(crate::jxl_decode::jxl_bitstream::Error::ProfileConformance("frame area too large").into());
        }
        ...
        if header.flags.use_lf_frame() && header.lf_level >= 4 {
            return Err(crate::jxl_decode::jxl_bitstream::Error::ValidationFailed("lf_level out of range").into());
        }
            ...
            .zip(image_header.metadata.ec_info.iter())
        {
            ...
            if actual_dim_shift > 7 + header.group_size_shift {
                return Err(crate::jxl_decode::jxl_bitstream::Error::ValidationFailed("dim_shift too large").into());
            }
        }
        ...
        if self.toc.is_single_entry() {
            ...
            if offset == 0 {
                ...
                let lf_group = self
                    ...
                    .ok_or(
                        crate::jxl_decode::jxl_bitstream::Error::Io(std::io::ErrorKind::UnexpectedEof.into()).into(),
                    )
                    .and_then(|x| x);
