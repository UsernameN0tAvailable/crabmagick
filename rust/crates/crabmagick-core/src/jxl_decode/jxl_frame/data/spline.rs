...
impl Bundle<&FrameHeader> for Splines {
    ...
    fn parse(bitstream: &mut Bitstream, header: &FrameHeader) -> Result<Self> {
        let mut decoder = crate::jxl_decode::jxl_coding::Decoder::parse(bitstream, 6)?;
        ...
        if num_splines >= max_num_splines {
            ...
            return Err(crate::jxl_decode::jxl_bitstream::Error::ProfileConformance("too many splines").into());
        }
...
impl Bundle<QuantSplineParams<'_>> for QuantSpline {
    ...
    fn parse(
        ...
        if acc_num_points > max_num_points {
            ...
            return Err(crate::jxl_decode::jxl_bitstream::Error::ProfileConformance("too many spline points").into());
        }
        ...
        for _ in 0..num_points {
            ...
            cur_value.0 = cur_value.0.checked_add(cur_delta.0).ok_or(
                crate::jxl_decode::jxl_bitstream::Error::ValidationFailed("control point overflowed"),
            )?;
