...
use crate::jxl_decode::jxl_vardct::{HfBlockContext, LfChannelCorrelation, LfChannelDequantization, Quantizer};

use crate::jxl_decode::jxl_frame::{FrameHeader, Result, header::Encoding};
...
impl<S: Sample> Bundle<LfGlobalParams<'_, '_>> for LfGlobal<S> {
    ...
    fn parse(bitstream: &mut Bitstream, params: LfGlobalParams) -> Result<Self> {
        ...
        let vardct = (header.encoding == crate::jxl_decode::jxl_frame::header::Encoding::VarDct)
...
define_bundle! {
    #[derive(Debug)]
    pub struct LfGlobalVarDct error(crate::jxl_decode::jxl_frame::Error) {
