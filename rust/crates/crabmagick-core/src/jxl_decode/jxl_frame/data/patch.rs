...
impl Bundle<(&ImageHeader, &FrameHeader)> for Patches {
    ...
    fn parse(
        ...
        let mut decoder = crate::jxl_decode::jxl_coding::Decoder::parse(bitstream, 10)?;
        ...
        if num_patch_refs > max_num_patch_refs {
            ...
            return Err(crate::jxl_decode::jxl_bitstream::Error::ProfileConformance("too many patches").into());
        }
        ...
        let patches = std::iter::repeat_with(|| -> Result<_> {
            ...
            if ref_idx >= 4 {
                ...
                return Err(
                    crate::jxl_decode::jxl_bitstream::Error::ValidationFailed("PatchRef index out of bounds").into(),
                );
            }
            ...
            let patch_targets = std::iter::repeat_with(|| -> Result<_> {
                let (x, y) = if let Some((px, py)) = prev_xy {
                    ...
                    let (Some(x), Some(y)) = (x, y) else {
                        ...
                        return Err(
                            crate::jxl_decode::jxl_bitstream::Error::ValidationFailed("patch coord overflow").into(),
                        );
                    };
                    (x, y)
                } else {
