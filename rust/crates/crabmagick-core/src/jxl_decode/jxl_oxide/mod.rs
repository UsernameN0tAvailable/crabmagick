...
//! JXL decoder is a JPEG XL decoder written in pure Rust. It's internally organized into a few
...
//!
//! ```no_run
//! # use crate::jxl_decode::jxl_oxide::JxlImage;
...
//! #     type Output = crate::jxl_decode::jxl_oxide::Result<&'static [u8]>;
//! #     fn poll(
...
//! # }
//! #
//! # use crate::jxl_decode::jxl_oxide::{JxlImage, InitializeResult};
//! # async fn run() -> crate::jxl_decode::jxl_oxide::Result<()> {
...
//!
//! ```no_run
//! # use crate::jxl_decode::jxl_oxide::Render;
//! use crate::jxl_decode::jxl_oxide::{JxlImage, RenderResult};
//!
//! # fn present_image(_: Render) {}
//! # fn main() -> crate::jxl_decode::jxl_oxide::Result<()> {
...
//!
//! # Color management
//! JXL decoder has basic color management support, which enables color transformation between
//! well-known color encodings and parsing simple, matrix-based ICC profiles. However, JXL decoder
...
//!
//! ```no_run
//! # use crate::jxl_decode::jxl_oxide::{EnumColourEncoding, JxlImage, RenderingIntent};
//! # use crate::jxl_decode::jxl_oxide::NullCms as MyCustomCms;
//! # let reader = std::io::empty();
...
//!
//! ```no_run
//! # use crate::jxl_decode::jxl_oxide::{JxlImage, NullCms};
//! # let reader = std::io::empty();
//! let mut image = JxlImage::builder().read(reader).expect("Failed to read image header");
...
use crate::jxl_decode::jxl_bitstream::{Bitstream, ContainerParser, ParseEvent};
use crate::jxl_decode::jxl_frame::FrameContext;
use crate::jxl_decode::jxl_image::BitDepth;
use crate::jxl_decode::jxl_oxide_common::{Bundle, Name};
use crate::jxl_decode::jxl_render::ImageBuffer;
use crate::jxl_decode::jxl_render::ImageWithRegion;
...
pub use crate::jxl_decode::jxl_image::color::{self, EnumColourEncoding, RenderingIntent};
pub use crate::jxl_decode::jxl_image::{self as image, ExtraChannelType, ImageHeader};
pub use crate::jxl_decode::jxl_jbr as jpeg_bitstream;
pub use crate::jxl_decode::jxl_threadpool::JxlThreadPool;
...
/// # Examples
/// ```no_run
/// # fn read_bytes() -> crate::jxl_decode::jxl_oxide::Result<&'static [u8]> { Ok(&[]) }
/// # use crate::jxl_decode::jxl_oxide::{JxlImage, InitializeResult};
/// # fn main() -> crate::jxl_decode::jxl_oxide::Result<()> {
/// let mut uninit_image = JxlImage::builder().build_uninit();
/// let image = loop {
...
impl UninitializedJxlImage {
    ...
    pub fn try_init(mut self) -> Result<InitializeResult> {
        ...
        let embedded_icc = if image_header.metadata.colour_encoding.want_icc() {
            let icc = match crate::jxl_decode::jxl_color::icc::read_icc(&mut bitstream) {
                Ok(x) => x,
                Err(e) if e.unexpected_eof() => {
                    return Ok(InitializeResult::NeedMoreData(self));
                }
            ...
            let icc = crate::jxl_decode::jxl_color::icc::decode_icc(&icc)?;
            Some(icc)
        } else {
            None
        };
    ...
    pub fn rendered_icc(&self) -> Vec<u8> {
        ...
        match encoding.encoding() {
            color::ColourEncoding::Enum(encoding) => {
                crate::jxl_decode::jxl_color::icc::colour_encoding_to_icc(encoding)
            }
...
impl JxlImage {
    ...
    pub fn jpeg_reconstruction_status(&self) -> JpegReconstructionStatus {
        match self.inner.aux_boxes.jbrd() {
            ...
            AuxBoxData::Decoding => {
                ...
                let frame_header = frame.header();
                if frame_header.encoding != crate::jxl_decode::jxl_frame::header::Encoding::VarDct {
                    return JpegReconstructionStatus::Invalid;
                }
    ...
    pub fn reconstruct_jpeg(&self, jpeg_output: impl std::io::Write) -> Result<()> {
        ...
        let jbrd = match aux_boxes.jbrd() {
            ...
            AuxBoxData::Decoding => {
                return Err(crate::jxl_decode::jxl_jbr::Error::ReconstructionDataIncomplete.into());
            }
            AuxBoxData::NotFound => {
                return Err(crate::jxl_decode::jxl_jbr::Error::ReconstructionUnavailable.into());
            }
        };
        if self.num_loaded_frames() == 0 {
            return Err(crate::jxl_decode::jxl_jbr::Error::FrameDataIncomplete.into());
        }
...
impl From<CropInfo> for crate::jxl_decode::jxl_render::Region {
...
impl From<crate::jxl_decode::jxl_render::Region> for CropInfo {
    fn from(value: crate::jxl_decode::jxl_render::Region) -> Self {
        Self {
