...
pub fn read_permutation(
    ...
    decoder: &mut crate::jxl_decode::jxl_coding::Decoder,
    size: u32,
    skip: u32,
) -> crate::jxl_decode::jxl_coding::CodingResult<Vec<usize>> {
    let end = decoder.read_varint(bitstream, get_context(size))?;
    if end > size - skip {
        tracing::error!(size, skip, end, "Invalid permutation");
        return Err(crate::jxl_decode::jxl_coding::Error::InvalidPermutation);
    }
...
fn get_context(x: u32) -> u32 {
    crate::jxl_decode::jxl_coding::add_log2_ceil(x).min(7)
}
