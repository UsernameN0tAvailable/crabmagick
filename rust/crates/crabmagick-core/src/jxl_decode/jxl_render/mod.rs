...
//! This crate is the core of JXL decoder that provides JPEG XL renderer.
...
use crate::jxl_decode::jxl_image::{ImageHeader, ImageMetadata};
...
impl RenderContextBuilder {
    ...
    pub fn build(self, image_header: Arc<ImageHeader>) -> Result<RenderContext> {
        ...
        Ok(RenderContext {
            ...
            cms: Box::new(crate::jxl_decode::jxl_color::NullCms),
    ...
    pub fn suggested_hdr_tf(&self) -> Option<TransferFunction> {
        let tf = match &self.image_header.metadata.colour_encoding {
            ...
            ColourEncoding::IccProfile(_) => {
                ...
                crate::jxl_decode::jxl_color::icc::icc_tf(icc)?
            }
        };
