...
use crate::jxl_decode::jxl_modular::{ChannelShift, Sample, image::TransformedModularSubimage};
...
pub(crate) fn load_lf_groups<S: Sample>(
    ...
    let mut lf_xyb = if lf_global_vardct.is_some() && !frame_header.flags.use_lf_frame() {
        ...
        for shift in shifts_cbycr {
            ...
            // SAFETY: copy_lf_dequant writes every element of this buffer before it is read.
            let buffer = unsafe { AlignedGrid::<f32>::uninit(width as usize, height as usize, tracker)? };
...
pub(crate) fn convert_color_for_record(
    ...
    if do_ycbcr {
        ...
        crate::jxl_decode::jxl_color::ycbcr_to_rgb([cb.buf_mut(), y.buf_mut(), cr.buf_mut()]);
...
impl<S: Default + Clone> PaddedGrid<S> {
    pub fn with_alloc_tracker(
        ...
        height: usize,
        padding: usize,
        tracker: Option<&crate::jxl_decode::jxl_grid::AllocTracker>,
    ) -> Result<Self> {
        Ok(Self {
