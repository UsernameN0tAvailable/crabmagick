...
use crate::jxl_decode::jxl_frame::{FrameHeader, data::GlobalModular};
use crate::jxl_decode::jxl_grid::{AlignedGrid, AllocTracker, MutableSubgrid};
use crate::jxl_decode::jxl_image::{BitDepth, ImageHeader};
use crate::jxl_decode::jxl_modular::{ChannelShift, Sample};
use crate::jxl_decode::jxl_threadpool::JxlThreadPool;
use crate::jxl_decode::jxl_vardct::LfChannelDequantization;

use crate::jxl_decode::jxl_render::{FrameRender, FrameRenderHandle, IndexedFrame, Region, Result, util};
...
impl ImageWithRegion {
    ...
    pub(crate) fn upsample_jpeg(
        ...
        for (g, (region, shift)) in self.buffer.iter_mut().zip(&mut self.regions).take(3) {
            ...
            let out = crate::jxl_decode::jxl_render::filter::apply_jpeg_upsampling_single(
                subgrid,
                *shift,
                valid_region,
                self.tracker.as_ref(),
    ...
    pub(crate) fn upsample_nonseparable(
        ...
        for (idx, ((region, shift), g)) in self.regions.iter_mut().zip(&mut self.buffer).enumerate()
        {
            ...
            *region = downsampled_valid_region;
            ...
            .in_scope(|| {
                crate::jxl_decode::jxl_render::features::upsample(
                    subgrid,
                    region,
                    image_header,
    ...
    pub(crate) fn fill_opaque_alpha(&mut self, ec_info: &[crate::jxl_decode::jxl_image::ExtraChannelInfo]) {
        for (ec_idx, ec_info) in ec_info.iter().enumerate() {
            if !matches!(ec_info.ty, crate::jxl_decode::jxl_image::ExtraChannelType::Alpha { .. }) {
                continue;
            }
...
impl ImageWithRegion {
    ...
    pub(crate) fn color_groups_with_group_id(
        &mut self,
        frame_header: &crate::jxl_decode::jxl_frame::FrameHeader,
...
pub(crate) fn composite<S: Sample>(
    frame: &IndexedFrame,
    grid: &mut ImageWithRegion,
    refs: [Option<crate::jxl_decode::jxl_render::Reference<S>>; 4],
    ...
    let image = crate::jxl_decode::jxl_render::blend::blend(image_header, refs, frame, grid, frame_region, pool)?;
    *grid = image;
    Ok(())
}
