...
pub struct HfPassParams<'a> {
    hf_block_ctx: &'a crate::jxl_decode::jxl_vardct::HfBlockContext,
    num_hf_presets: u32,
}

impl<'a> HfPassParams<'a> {
    pub fn new(hf_block_ctx: &'a crate::jxl_decode::jxl_vardct::HfBlockContext, num_hf_presets: u32) -> Self {
        Self {
            hf_block_ctx,
            num_hf_presets,
        }
...
/// passed as a parameter when [decoding HF coefficients][crate::jxl_decode::jxl_vardct::write_hf_coeff].
#[derive(Debug)]
pub struct HfPass {
...
impl Bundle<HfPassParams<'_>> for HfPass {
    ...
    fn parse(bitstream: &mut Bitstream, params: HfPassParams<'_>) -> crate::jxl_decode::jxl_vardct::Result<Self> {
        let HfPassParams {
            hf_block_ctx,
            num_hf_presets,
        } = params;
            ...
            for (idx, (permutation, (bw, bh))) in it {
                if used_orders & 1 != 0 {
                    ...
                    for permutation in permutation {
                        let perm = crate::jxl_decode::jxl_coding::read_permutation(bitstream, decoder, size, skip)?;
