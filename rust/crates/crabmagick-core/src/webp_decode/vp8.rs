...
use crate::webp_decode::decoder::{DecodingError, UpsamplingMethod};
use crate::webp_decode::yuv;

use super::vp8_arithmetic_decoder::ArithmeticDecoder;
use super::{loop_filter, transform};

/// Pre-computed per-macroblock loop filter parameters.
struct FilterParams {
    filter_level: u8,
    interior_limit: u8,
    hev_threshold: u8,
...
/// Thin wrapper around *mut u8 to make it Send/Sync for rayon.
/// SAFETY: caller must guarantee non-overlapping access.
#[derive(Copy, Clone)]
...
impl SendPtr {
    /// Extract the raw pointer. Calling this as a method forces the closure to
    /// capture the whole `SendPtr` (which is `Send+Sync`) rather than just the
    /// inner `*mut u8` field (which is `!Send+!Sync`) under Rust 2021 capture rules.
    #[inline(always)]
    fn ptr(self) -> *mut u8 {
        self.0
    }
...
/// Apply the loop filter for a single macroblock using raw pixel-buffer pointers.
///
/// # Safety
/// `ybuf`, `ubuf`, `vbuf` must each point to the start of the respective
/// fully-allocated plane buffer.  Caller must guarantee that concurrent calls
/// for different MB rows access disjoint regions of those buffers.
#[allow(clippy::too_many_arguments)]
unsafe fn apply_loop_filter_mb(
    mbx: usize,
    mby: usize,
    fp: &FilterParams,
    filter_type: bool,
    ...
    if fp.filter_level == 0 {
        return;
    }

    let ybuf = unsafe { std::slice::from_raw_parts_mut(ybuf, ybuf_len) };
    let ubuf = unsafe { std::slice::from_raw_parts_mut(ubuf, ubuf_len) };
    let vbuf = unsafe { std::slice::from_raw_parts_mut(vbuf, vbuf_len) };
        ...
        if filter_type {
            ...
            for y in 0usize..8 {
                ...
                loop_filter::macroblock_filter_horizontal(
                    ...
                    interior_limit,
                    mbedge_limit,
                    &mut ubuf[y0 * chroma_w + x0 - 4..][..8],
                );
                loop_filter::macroblock_filter_horizontal(
                    hev_threshold,
                    interior_limit,
                    mbedge_limit,
                    &mut vbuf[y0 * chroma_w + x0 - 4..][..8],
                );
            }
        }
    }
    ...
    if mby > 0 {
        if filter_type {
            for x in 0usize..16 {
                let y0 = mby * 16;
                let x0 = mbx * 16 + x;
                loop_filter::simple_segment_vertical(mbedge_limit, ybuf, y0 * luma_w + x0, luma_w);
            }
        } else {
            for x in 0usize..16 {
                let y0 = mby * 16;
...
impl<R: Read> Vp8Decoder<R> {
    ...
    /// Pre-compute all per-MB loop filter parameters into a flat Vec.
    fn precompute_filter_params(&self) -> Vec<FilterParams> {
        let mbw = self.mbwidth as usize;
        let mbh = self.mbheight as usize;
        let mut out = Vec::with_capacity(mbw * mbh);
        for mby in 0..mbh {
            for mbx in 0..mbw {
                let mb = &self.macroblocks[mby * mbw + mbx];
    ...
    fn decode_frame_(mut self) -> Result<Frame, DecodingError> {
        ...
        let t_recon_start = std::time::Instant::now();
        let mut t_arith_ns = 0u64;
        let mut t_intra_ns = 0u64;
        ...
        let t_recon_done = t_recon_start.elapsed();

        // ── Phase 2: loop filtering ──────────────────────────────────────────
        // Two-phase parallel: even rows first, then odd rows.
        // MB rows 2+ apart never share pixel regions (gap ≥ 12 rows between
        // filter extents), so each phase is safe to run in parallel.
        let filter_params = self.precompute_filter_params();
        let luma_w = mbwidth * 16;
        let chroma_w = mbwidth * 8;
        let filter_type = self.frame.filter_type;
        let ybuf_len = self.frame.ybuf.len();
        let ubuf_len = self.frame.ubuf.len();
        let vbuf_len = self.frame.vbuf.len();
        let ybuf = SendPtr(self.frame.ybuf.as_mut_ptr());
        let ubuf = SendPtr(self.frame.ubuf.as_mut_ptr());
        let vbuf = SendPtr(self.frame.vbuf.as_mut_ptr());

        let t_lf_start = std::time::Instant::now();
        for phase in 0..2usize {
            (phase..mbheight)
                ...
                .for_each(|mby| {
                    for mbx in 0..mbwidth {
                        let fp = &filter_params[mby * mbwidth + mbx];
                        // SAFETY: rows within the same phase are ≥2 apart, so
                        // their filter pixel regions ([mby*16-4, mby*16+16) for Y,
                        // [mby*8-2, mby*8+8) for UV) are disjoint.
                        unsafe {
                            apply_loop_filter_mb(
                                mbx,
                                mby,
                                fp,
                                filter_type,
        ...
        let t_lf_done = t_lf_start.elapsed();
        if std::env::var_os("IIIF_TIMING").is_some() {
            eprintln!(
                "WebP decoder internal: recon={}ms lf={}ms arith={}ms intra={}ms ({}×{}MBs)",
                t_recon_done.as_millis(),
                t_lf_done.as_millis(),
                t_arith_ns / 1_000_000,
                t_intra_ns / 1_000_000,
                mbwidth,
                mbheight,
            );
        }
